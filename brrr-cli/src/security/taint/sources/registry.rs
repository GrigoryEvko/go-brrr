...
pub struct SourceRegistry {
    ...
    by_label: FxHashMap<TaintLabel, Vec<usize>>,
}
...
pub fn get_python_sources() -> SourceRegistry {
    ...
    registry.add(TaintSource::prefix(
        "request.args",
    ...
    registry.add(TaintSource::exact(
        "request.path",
        TaintLabel::UrlData,
        "Flask URL path",
    ));
    ...
    // FastAPI sources
    registry.add(TaintSource::regex(
        r"\bQuery\(",
        TaintLabel::UserInput,
        "FastAPI Query",
    ));
    ...
    // File operations
    registry
        .add(TaintSource::exact("open", TaintLabel::FileContent, "File open").with_confidence(0.7));
    registry.add(TaintSource::suffix(
        ".read()",
    ...
    registry.add(TaintSource::suffix(
        ".json()",
        TaintLabel::ExternalApi,
        "Response JSON",
    ));
    registry.add(
        TaintSource::suffix(".text", TaintLabel::ExternalApi, "Response text").with_confidence(0.7),
    );
...
pub fn get_typescript_sources() -> SourceRegistry {
    ...
    registry.add(
        TaintSource::suffix(".value", TaintLabel::UserInput, "DOM element value")
            .with_confidence(0.6),
    );
    registry.add(
        TaintSource::suffix(".innerHTML", TaintLabel::UserInput, "DOM innerHTML")
            .with_confidence(0.5),
    );
    registry.add(TaintSource::exact(
        "FormData",
    ...
    // Network operations
    registry.add(
        TaintSource::exact("fetch", TaintLabel::NetworkData, "Fetch API").with_confidence(0.8),
    );
    registry.add(TaintSource::prefix(
        "axios.",
        TaintLabel::NetworkData,
    ...
    // WebSocket
    registry.add(
        TaintSource::suffix(".onmessage", TaintLabel::NetworkData, "WebSocket message")
            .with_confidence(0.7),
    );
    registry.add(
        TaintSource::suffix("event.data", TaintLabel::NetworkData, "WebSocket data")
            .with_confidence(0.7),
    );
...
pub fn get_go_sources() -> SourceRegistry {
    ...
    registry.add(
        TaintSource::suffix(".Read(", TaintLabel::FileContent, "Read from reader")
            .with_confidence(0.5),
    );
    registry.add(TaintSource::suffix(
        ".ReadAll(",
...
pub fn get_rust_sources() -> SourceRegistry {
    ...
    // Axum framework
    registry.add(
        TaintSource::exact("Query", TaintLabel::UserInput, "Axum query params")
            .with_confidence(0.8),
    );
    registry.add(
        TaintSource::exact("Form", TaintLabel::UserInput, "Axum form data").with_confidence(0.8),
    );
    registry.add(
        TaintSource::exact("Json", TaintLabel::UserInput, "Axum JSON body").with_confidence(0.8),
    );
    registry.add(
        TaintSource::exact("Path", TaintLabel::UrlData, "Axum path params").with_confidence(0.8),
    );
    registry.add(TaintSource::exact(
        "TypedHeader",
        TaintLabel::HttpHeader,
