...
pub(crate) const RULES: &[RuleDef] = &[
    ...
    // ── TypeSafe (D3390 §1.5.2 + §2.5) ────────────────────────────────────
    RuleDef {
        id: "TSAF-001",
        axiom: SafetyAxiom::TypeSafe,
        severity: Severity::High,
        confidence: Confidence::Medium,
        title: "std::optional accessed without has_value() check",
        description: "Accessing std::optional via operator*, operator->, or .value() without a prior has_value() check or boolean test is undefined behavior when the optional is empty. D3390 §2.5 requires discriminant validation.",
        remediation: "Guard with `if (opt.has_value())` or `if (opt)` before access, or use `.value_or(default)`",
        cwe: Some(476),
    },
    RuleDef {
        id: "TSAF-002",
        ...
        severity: Severity::High,
        confidence: Confidence::Medium,
        title: "std::expected accessed without has_value() check",
        description: "Accessing std::expected via .value() or operator* without a prior has_value() check throws bad_expected_access or causes UB. D3390 §2.5 requires discriminant validation.",
        remediation: "Guard with `if (ex.has_value())` before calling .value(), or use .value_or(default)",
        cwe: Some(476),
    },
    RuleDef {
        id: "TSAF-003",
        ...
        severity: Severity::High,
        confidence: Confidence::Medium,
        title: "std::variant accessed via std::get without holds_alternative check",
        description: "std::get<T>(v) throws bad_variant_access if the variant doesn't hold T. D3390 §2.5 requires discriminant validation. Use std::visit for safe exhaustive access.",
        remediation: "Use `std::visit()` for exhaustive handling, or guard with `std::holds_alternative<T>(v)` before std::get",
        cwe: Some(843),
    },
    RuleDef {
        id: "TSAF-004",
        ...
        severity: Severity::Medium,
        confidence: Confidence::Low,
        title: "Smart pointer dereferenced without null check",
        description: "std::unique_ptr and std::shared_ptr can be null. Dereferencing via operator* or operator-> without a prior null check is UB. The billion-dollar mistake applies to smart pointers too.",
        remediation: "Guard with `if (ptr)` or `if (ptr != nullptr)` before dereferencing",
        cwe: Some(476),
    },
    RuleDef {
        id: "TSAF-005",
        ...
        severity: Severity::High,
        confidence: Confidence::Medium,
        title: "std::any accessed via any_cast without type check",
        description: "std::any_cast<T>(a) throws bad_any_cast if the any doesn't hold T. The pointer form returns nullptr on mismatch. D3390 treats unchecked type-erased access as unsafe.",
        remediation: "Check with `a.type() == typeid(T)` before any_cast, or use the pointer form and check for nullptr",
        cwe: Some(843),
    },
    // ── TypeSafe (D3390 §2.5.1 — Pattern Matching) ──────────────────────
    RuleDef {
        id: "TSAF-006",
        axiom: SafetyAxiom::TypeSafe,
        severity: Severity::Medium,
        confidence: Confidence::Medium,
        title: "switch on enum without default case",
        description: "Switch on an enum-typed expression without a default case risks silently skipping values added in future. D3390 §2.5.1 requires exhaustive pattern matching.",
        remediation: "Add a `default:` case (ideally with `assert(false)` or `std::unreachable()`) or use if-else chain",
        cwe: Some(478),
    },
    RuleDef {
        id: "TSAF-007",
        ...
        severity: Severity::Medium,
        confidence: Confidence::High,
        title: "switch case implicit fallthrough",
        description: "Case block ends without break, return, throw, continue, goto, or [[fallthrough]]. Implicit fallthrough is one of the most common C/C++ bug sources.",
        remediation: "Add `break;` at end of case, or annotate with `[[fallthrough]];` if intentional",
        cwe: Some(484),
    },
    RuleDef {
        id: "TSAF-008",
        ...
        severity: Severity::Low,
        confidence: Confidence::High,
        title: "switch on boolean value",
        description: "switch on a bool has only two states and is better expressed as if/else. This pattern obscures intent and is error-prone.",
        remediation: "Replace `switch(b)` with `if (b) { ... } else { ... }`",
        cwe: None,
    },
    RuleDef {
        id: "TSAF-009",
        ...
        description: "Switch statement has no default case. If the switched value doesn't match any case, execution silently falls through. D3390 §2.5.1 requires exhaustive handling.",
        remediation: "Add a `default:` case to handle unexpected values, or use `[[assume(false)]]` to assert exhaustiveness",
        cwe: Some(478),
    },
];
