...
use brrr::callgraph::indexer::FunctionIndex;
use brrr::callgraph::types::{CallEdge, CallGraph, FunctionRef};
use brrr::callgraph::{analyze_dead_code, analyze_impact, build, DeadCodeConfig, ImpactConfig};
...
fn bench_call_graph_queries(c: &mut Criterion) {
    ...
    group.bench_function("dead_code_with_config", |b| {
        ...
        b.iter(|| {
            let result =
                brrr::callgraph::dead::analyze_dead_code_with_config(black_box(&graph), &config);
            black_box(result)
        });
    });
