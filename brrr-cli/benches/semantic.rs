...
use brrr::embedding::{Metric, VectorIndex};
use brrr::semantic::{
...
use brrr::util::tokenizer::{estimate_tokens, fits_in_tokens, split_at_tokens};
...
fn bench_extract_units_python(c: &mut Criterion) {
    ...
    group.bench_function("medium_class", |b| {
        b.iter(|| brrr::extract_file_units(black_box(&path)))
    });
    ...
    group.bench_function("large_file", |b| {
        b.iter(|| brrr::extract_file_units(black_box(&large_path)))
    });
...
fn bench_full_embedding_pipeline(c: &mut Criterion) {
    ...
    group.bench_function("extract_build_count", |b| {
        b.iter(|| {
            let units = brrr::extract_file_units(black_box(&path)).unwrap_or_default();
            for unit in &units {
