...
fn rule_why(code: RuleCode) -> &'static str {
    match code {
        ...
        RuleCode::FST020 => "F* parses `(f: a -> b)` as `(f:a) -> b`, not a function-typed parameter.",
        RuleCode::FST021 => "F* reserved keywords as identifiers cause cryptic parse errors.",
        RuleCode::FST022 => "Records with function fields need `noeq` or equality derivation fails (Error 76).",
        RuleCode::FST023 => "Unguarded forall without SMTPat causes Z3 divergence or instability.",
        RuleCode::FST024 => "Decreases clause referencing unbound name silently creates fresh variable.",
        RuleCode::FST025 => "`assume type` and `assume val` have different semantics; confusion creates unsound axioms.",
        RuleCode::FST026 => "`reveal` requires ghost context (GTot); using in Tot causes effect mismatch.",
        RuleCode::FST027 => "Without `open FStar.Mul`, `*` means tuple type, not integer multiplication.",
        RuleCode::FST028 => "Matching on implicits without `strict_on_arguments` prevents reduction.",
        RuleCode::FST029 => "Pattern `\\/` is disjunctive trigger, not logical OR — unexpected SMT behavior.",
        RuleCode::FST030 => "Function equality is undecidable; direct comparison always fails.",
        RuleCode::FST031 => "An `opaque_to_smt` definition never revealed is invisible to Z3.",
        RuleCode::FST032 => "Inconsistent universe annotations cause unexpected polymorphism behavior.",
        RuleCode::FST033 => "Dedicated tactics (norm_num, linarith, ring) are faster and more stable than raw SMT.",
        RuleCode::FST034 => "Equality lemmas with `[@@simp]` reduce manual proof effort via automatic rewriting.",
        RuleCode::FST035 => "Commutativity lemmas without ordering guards cause infinite simplifier loops.",
        RuleCode::FST036 => "`$f` binder prevents normalization, preserving structure for proof terms.",
        RuleCode::FST037 => "Tot/GTot mismatch in erased positions causes effect boundary violations.",
        RuleCode::FST038 => "`introduce forall/implies ... with` is cleaner than manual intro tactics.",
        RuleCode::FST039 => "Type aliases without `unfold` are opaque to the normalizer, causing mismatches.",
        RuleCode::FST040 => "`@@@` targets declarations, `@@` targets terms; wrong one silently misattaches.",
        RuleCode::FST041 => "`requires True` is intentional in Stack/ST signatures — not redundant.",
        RuleCode::FST042 => "`Lemma post` means `requires post, ensures True` — opposite of intended.",
        RuleCode::FST043 => "Mixing tick-variables and explicit implicits reduces readability.",
        RuleCode::FST044 => "Non-structural recursion without `decreases` may fail to verify.",
        RuleCode::FST045 => "`unopteq` signals optional equality; `noeq` is too restrictive for simple records.",
        RuleCode::FST046 => "Proof-only types without `erasable` bloat extracted code unnecessarily.",
        RuleCode::FST047 => "`Prims.sum` (constructive) and `\\/` (propositional) have incompatible use cases.",
        RuleCode::FST048 => "Complex tactics without `[@@plugin]` run interpreted, causing 10-100x slowdown.",
        RuleCode::FST049 => "`auto_safe` vs `auto_unsafe` classification prevents expensive lemma misfires.",
        RuleCode::FST050 => "Manual squash↔refinement coercion is error-prone; use Squash module bridges.",
        RuleCode::FST051 => "Without `do_not_unrefine`, F* may silently drop refinement invariants.",
    }
}
...
fn rule_how_to_fix(code: RuleCode) -> &'static str {
    match code {
        ...
        RuleCode::FST020 => "Wrap function-typed parameters in double parens: `(f: (a -> b))`.",
        RuleCode::FST021 => "Rename the identifier to avoid the F* reserved keyword.",
        RuleCode::FST022 => "Add `noeq` qualifier before `type`: `noeq type my_record = { ... }`.",
        RuleCode::FST023 => "Add `[SMTPat (...)]` trigger or use `assert_norm`/tactics instead.",
        RuleCode::FST024 => "Fix the decreases clause to reference an actual function parameter.",
        RuleCode::FST025 => "Use `assume val` for value axioms, `assume type` only for abstract types.",
        RuleCode::FST026 => "Move `reveal` into a Ghost/Lemma context or use `Ghost.elim_pure`.",
        RuleCode::FST027 => "Add `open FStar.Mul` at the top of the module.",
        RuleCode::FST028 => "Add `[@@ strict_on_arguments [N]]` where N is the implicit argument index.",
        RuleCode::FST029 => "Use separate `[SMTPat ...]` entries for alternative patterns.",
        RuleCode::FST030 => "Use `feq f g` or `on_domain t f == on_domain t g` from FunctionalExtensionality.",
        RuleCode::FST031 => "Add `reveal_opaque ``name` where the definition's content is needed.",
        RuleCode::FST032 => "Add explicit universe annotations: `val f : #u:universe -> Type u#u -> ...`.",
        RuleCode::FST033 => "Replace `assert (...)` with `assert (...) by (tactic_name ())`.",
        RuleCode::FST034 => "Add `[@@simp]` attribute to the lemma declaration.",
        RuleCode::FST035 => "Add an ordering guard: `requires a <= b` to prevent infinite rewriting.",
        RuleCode::FST036 => "Change `(f: a -> Lemma ...)` to `($f: a -> Lemma ...)`.",
        RuleCode::FST037 => "Change the return effect to GTot, or restructure to avoid ghost/pure crossing.",
        RuleCode::FST038 => "Use `introduce forall x. P x with proof_body`.",
        RuleCode::FST039 => "Add `unfold` qualifier: `unfold let my_type = existing_type`.",
        RuleCode::FST040 => "Use `@@@` for val/let-level, `@@` for term-level attributes.",
        RuleCode::FST041 => "No action needed — `requires True` is intentional here.",
        RuleCode::FST042 => "Change `Lemma post` to `Lemma (ensures post)`.",
        RuleCode::FST043 => "Pick one style and use it consistently within the module.",
        RuleCode::FST044 => "Add `(decreases expr)` clause to the recursive function.",
        RuleCode::FST045 => "Replace `noeq` with `unopteq` on this record type.",
        RuleCode::FST046 => "Add `[@@ erasable]` attribute to the type definition.",
        RuleCode::FST047 => "Use `\\/` for proofs, `Prims.sum`/`either` for computation.",
        RuleCode::FST048 => "Add `[@@plugin]` attribute to the tactic definition.",
        RuleCode::FST049 => "Classify as `[@@auto_safe]` (cheap, always useful) or `[@@auto_unsafe]` (expensive).",
        RuleCode::FST050 => "Use `FStar.Squash.give_proof`/`get_proof` for safe squash bridging.",
        RuleCode::FST051 => "Add `[@@do_not_unrefine]` attribute to the refined type alias.",
    }
}
