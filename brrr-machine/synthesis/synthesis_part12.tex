...
\begin{remarkbox}[title={Historical Note on Honda 2008 Proofs}]
\textbf{Important:} The original Honda et al.\ 2008 proofs for multiparty session types were found to be \textbf{flawed} by Scalas \& Yoshida (2019). Two corrected approaches now exist:
\begin{enumerate}
\item \textbf{Bottom-up (Scalas 2019)}: Uses safety property $\varphi$, no global types required for safety
\item \textbf{Top-down (Yoshida 2024)}: Uses association relation with subtyping: $G \upharpoonright p \leq \Gamma(s[p])$
\end{enumerate}
\textbf{Note:} Honda 1998 (binary session types) remains sound and has been mechanized (LinearActris, POPL 2024). See Part XIV Section 14.3.1 for full details on the corrections.
\end{remarkbox}
