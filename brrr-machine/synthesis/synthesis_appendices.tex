...
\textbf{Honda 2008} & \textbf{9} & \textbf{Session Types} & \textbf{Multiparty asynchronous session types, global types, projection (see caveats below)} \\
\textbf{Scalas \& Yoshida 2019} & \textbf{9} & \textbf{Session Types} & \textbf{``Less is More'': Safety property, fixes Honda 2008 proof flaws, no global types needed} \\
\textbf{Yoshida \& Hou 2024} & \textbf{9} & \textbf{Session Types} & \textbf{``Less is More Revisited'': Association relation, validates top-down approach with subtyping} \\
\textbf{Tirore et al.~2023/2025} & \textbf{8} & \textbf{Session Types} & \textbf{Mechanized MPST in Coq (ITP 2023) and Rocq (ECOOP 2025), restricted fragment} \\
...
\item[Honda 2008 (9/10)] Extends binary session types to multiparty asynchronous sessions. Global types describe complete protocol scenarios; projection extracts local types for each participant. Critical for microservice protocol analysis and distributed system verification. \textbf{Caveat}: The original proofs had flaws discovered by Scalas \& Yoshida 2019; use the association relation (Yoshida \& Hou 2024) with subtyping rather than direct equality for sound type safety proofs.

\item[Scalas \& Yoshida 2019 (9/10)] ``Less is More: Multiparty Session Types Revisited'' (POPL 2019). Identified that Honda 2008's consistency requirement is overly restrictive and papers using ``full merging'' projection had flawed proofs. Introduces the \textbf{safety property} $\varphi$ as a weaker but sufficient condition for type safety. Key insight: global types are \textbf{not required} --- safety, deadlock-freedom, and liveness can be checked directly on typing contexts. Types more protocols than the original Honda approach.

\item[Yoshida \& Hou 2024 (9/10)] ``Less is More Revisited: Association with Global Multiparty Session Types'' (arXiv 2024). Corrects the misunderstanding that the top-down MPST approach with mergeability is unsound. Introduces the \textbf{association relation} $G \sqsubseteq_s \Gamma$ using subtyping ($G \upharpoonright p \leq \Gamma(s[p])$) rather than equality. Proves top-down approach is sound and decidable. Key result: association guarantees safety, deadlock-freedom, and liveness by construction.

\item[Tirore et al.~2023/2025 (8/10)] Mechanized proofs of multiparty session types. ITP 2023 provides Coq mechanization of a restricted fragment. ECOOP 2025 extends to full Rocq formalization. Useful reference for verification but covers a more limited fragment than the full Honda 2008 theory.
...
\item[Coherence] Honda 2008. Property that a global type is linear and projectable to all participants. Ensures protocol is implementable. \textbf{Note}: The original definition required exact equality between projections and local types; the corrected formulation (Yoshida \& Hou 2024) uses the association relation with subtyping.

\item[Consistency (Session Types)] Honda 2008. Requirement that all partial projections be dual. \textbf{Caveat}: Shown to be overly restrictive by Scalas \& Yoshida 2019; replaced by the weaker \textbf{safety property} in modern formulations.
...
\item[Projection (Session Types)] Honda 2008. Operation $G \restriction p$ extracting participant $p$'s local type from global type $G$. \textbf{Note}: In the corrected formulation (Yoshida \& Hou 2024), projections should be related to typing contexts via subtyping, not equality.

\item[Association Relation] Yoshida \& Hou 2024. Relation $G \sqsubseteq_s \Gamma$ connecting global types to typing contexts via subtyping: $G \upharpoonright p \leq \Gamma(s[p])$. Replaces direct equality in sound MPST proofs.

\item[Safety Property (MPST)] Scalas \& Yoshida 2019. Behavioral property $\varphi$ on typing contexts sufficient for type safety. Weaker than consistency but accepts more protocols. See Part XIV, Definition~\ref{def:safety-property}.
