...
For contexts $\env_1, \env_2$ resulting from splitting $\env$, we say they satisfy
\emph{linear exclusivity} if for all variables $x$ with linear extended mode:
\[
  (\env_1(x) = \mone \implies \env_2(x) = \mzero) \land
  (\env_2(x) = \mone \implies \env_1(x) = \mzero)
\]
That is, a linear resource can appear with mode $\mone$ in at most one branch.
\end{definition}

\begin{theorem}[Split Preserves Linear Exclusivity]
The $\mathsf{ctx\_split}$ operation produces contexts satisfying linear exclusivity:
\[
  \mathsf{ctx\_split}(\env) = (\env_1, \env_2) \implies
  \text{linear\_exclusive}(\env_1, \env_2)
\]
\end{theorem}

\begin{theorem}[Addition Preserves Validity Under Exclusivity]
If $\env_1, \env_2$ satisfy linear exclusivity, then $\env_1 + \env_2$ preserves
the validity invariant: for all $x$ with extended mode $\mathsf{EMLinear}$,
$(\env_1 + \env_2)(x) \in \{\mzero, \mone\}$.
\begin{proof}
By linear exclusivity, at most one of $\env_1(x), \env_2(x)$ equals $\mone$.
The possible cases for $\mathsf{mode\_add}$:
\begin{itemize}
  \item $\mzero + \mzero = \mzero \in \{\mzero, \mone\}$
  \item $\mzero + \mone = \mone \in \{\mzero, \mone\}$
  \item $\mone + \mzero = \mone \in \{\mzero, \mone\}$
\end{itemize}
The case $\mone + \mone = \momega$ cannot occur due to linear exclusivity.
\end{proof}
\end{theorem}
