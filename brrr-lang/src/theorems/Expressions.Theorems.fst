...
(* NOTE: We don't use 'friend Expressions' because it requires an interface file
   ...
   free_vars, etc. are all exported in Expressions.fsti. The helper lemmas for
...
(* NOTE: fresh_var_spec is already declared and proven in Expressions.fst,
   and exported via Expressions.fsti. The theorem fresh_var_spec can be
...
let theorem_fresh_var_spec base avoid =
  (* The specification is proven in Expressions.fst and exported via the interface.
...
let rec immediate_subexpr_size_lt (s: expr) (parent: expr)
  ...
  admit ()  (* Full proof requires subexpr_size_decreases from Expressions.fst *)
...
let theorem_free_vars_subexpr sub parent =
  ...
  let goal (v: var_id) : Lemma
    ...
    | EGoto _ | ESizeof _ | EAlignof _ | ETypeMethod _ _
...
let rec theorem_subst_expr_non_free var replacement target =
  ...
  (* ================================================================
     ...
     ================================================================ *)
  | ELit _ | EGlobal _ | EHole | EError _ | EContinue _ | EGoto _
  | ESizeof _ | EAlignof _ | ETypeMethod _ _ | EBreak _ None | EReturn None ->
 ...
 *
 * Reference: The fresh_var implementation in Expressions.fst produces names that
 ...
 *
 * Location: Expressions.fst (normalize_expr_equiv)
 * Status: PROVEN
 *
 * Proof Strategy:
...
let theorem_normalize_expr_equiv e =
  (* The theorem is proven in Expressions.fst via normalize_expr_equiv.
     The proof works by:
