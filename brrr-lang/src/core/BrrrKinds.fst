(**
 * BrrrLang.Core.Kinds - Implementation
 *
 ...
 * ============================================================================
 *
 * This file implements the KIND SYSTEM for Brrr-Lang, providing:
 *
 *   1. KIND TYPE DEFINITION - The grammar of kinds (KStar, KArrow, KRow, KRegion)
 *   2. KIND INFERENCE - Inferring the kind of any type expression
 *   3. KIND CHECKING - Verifying a type has an expected kind
 *   4. VARIANCE TRACKING - How subtyping propagates through type constructors
 *   5. HKT SUPPORT - Higher-kinded polymorphism for Functor/Monad/etc.
 *
 * ============================================================================
 * THEORETICAL BACKGROUND (Pierce TAPL Chapter 29)
 * ============================================================================
 *
 * This implementation follows the theory from:
 *   - Pierce, "Types and Programming Languages", Chapter 29 "Type Operators and Kinding"
 *
 * KEY CONCEPTS FROM TAPL:
 *
 *   Section 29.1 - KINDS AS TYPES OF TYPES:
 *   Just as types classify terms, kinds classify types. The kind * (KStar)
 *   is the kind of "proper types" - types that can classify values.
 *
 *   Section 29.2 - KINDING RELATION:
 *   The kinding judgment Delta |- T :: K says "type T has kind K in context Delta".
 *   Our infer_kind_env function computes K given Delta (kind_ctx) and T (brrr_type).
 *
 *   Section 29.3 - TYPE OPERATOR ABSTRACTION:
 *   Type constructors like Option are type-level lambdas:
 *     Option = Lambda a:*. None | Some a
 *   The kind of Option is * -> * (takes a type, returns a type).
 *
 *   Section 29.4 - HIGHER-KINDED POLYMORPHISM:
 *   Type variables can have non-* kinds, enabling:
 *     fmap : forall [F : KStar -> KStar]. Functor F => (a -> b) -> F a -> F b
 *   Here F has kind KStar -> KStar, and Functor has kind (KStar -> KStar) -> KStar.
 *
 * ============================================================================
 * KIND SYSTEM
 * ============================================================================
 ...
 *   - k1 -> k2 (KArrow)  = type constructor kind (takes k1, returns k2)
 ...
 * ============================================================================
 *
 *   - Option : KStar -> KStar                      (unary type constructor)
 *   - Result : KStar -> KStar -> KStar              (binary type constructor, curried)
 *   - Functor : (KStar -> KStar) -> KStar           (higher-kinded: takes type constructor)
 *   - Bifunctor : (KStar -> KStar -> KStar) -> KStar  (takes binary type constructor)
 *
 * The curried representation means Result is: KStar -> (KStar -> KStar)
 * So Result<A> has kind KStar -> KStar, and Result<A, E> has kind KStar.
 *
 * ============================================================================
 * HIGHER-KINDED TYPES FOR TYPECLASSES
 * ============================================================================
 *
 * This kind system enables Haskell/Scala-style typeclasses that abstract over
 ...
 *
 * Requires:
 *   - F to have kind KStar -> KStar (type constructor)
 *   - Functor to have kind (KStar -> KStar) -> KStar (takes constructor, returns type)
 *
 * The default_named_kind_env registers:
 *   - ("Functor", KArrow (KArrow KStar KStar) KStar)
 *   - ("Monad", KArrow (KArrow KStar KStar) KStar)
 *   - ("Applicative", KArrow (KArrow KStar KStar) KStar)
 *
 * See fstar_pop_book.md lines 12186-12357 for F* typeclass examples.
 *
 * ============================================================================
 * VARIANCE: SUBTYPING THROUGH TYPE CONSTRUCTORS
 * ============================================================================
 *
 * Variance (per TAPL Section 15.2) describes how subtyping propagates:
 *
 *   - COVARIANT (+): T <: U implies F<T> <: F<U>
 *     Example: Option<Dog> <: Option<Animal> if Dog <: Animal
 *
 *   - CONTRAVARIANT (-): T <: U implies F<U> <: F<T>
 *     Example: (Animal -> R) <: (Dog -> R) (function params are contravariant)
 *
 *   - INVARIANT (=): No subtyping relationship unless types are equal
 *     Example: Cell<T> is invariant (can read AND write)
 *
 *   - BIVARIANT: Unused positions (both covariant and contravariant)
 *
 * Variance composition: For F<G<T>>, multiply variances like signs.
 *
 * ============================================================================
...
(** ============================================================================
    KIND DEFINITION (per Pierce TAPL Chapter 29)
    ============================================================================

    The kind type forms the "type language" for our type system. Following
    TAPL Section 29.1, kinds classify types just as types classify values.

    KIND GRAMMAR (from brrr_lang_spec_v0.4.tex):
      kappa ::= * | kappa1 -> kappa2 | Row | Region

    SEMANTICS:
    - KStar: The kind of "proper types" - types that classify values.
             Int, String, Bool, Option<Int>, etc. all have kind KStar.
    - KArrow: The kind of type constructors. KArrow k1 k2 is the kind
              of a type operator that takes a k1-kinded type and returns
              a k2-kinded type.
    - KRow: The kind of effect rows (for row-polymorphic effects).
            Enables algebraic effects with effect handlers.
    - KRegion: The kind of lifetime/region annotations.
               Enables Rust-style lifetime polymorphism.

    HIGHER-ORDER KINDS (for typeclasses):
    - KArrow KStar KStar                              - Option, Vec, List
    - KArrow KStar (KArrow KStar KStar)               - Result, Either
    - KArrow (KArrow KStar KStar) KStar               - Functor, Monad
    - KArrow (KArrow KStar (KArrow KStar KStar)) KStar  - Bifunctor

    The arrow is right-associative: KStar -> KStar -> KStar means KStar -> (KStar -> KStar).
    ============================================================================ *)
...
(* ============================================================================
   ...
   CORE INSIGHT (Pierce TAPL Chapter 29):
   Just as TYPES classify VALUES, KINDS classify TYPES.
   This creates a two-level type system where we can reason about
   type constructors as first-class entities.

   THE SIMPLY-KINDED LAMBDA CALCULUS:
   Our kind system corresponds to the simply-kinded lambda calculus from TAPL:
   - KStar is the base kind (analogous to base types in STLC)
   - KArrow is the function kind (analogous to arrow types)
   - KRow/KRegion are domain-specific base kinds (extensions)

   TYPE-LEVEL LAMBDA CALCULUS:
   Type constructors are "type-level lambdas":
     Option = Lambda a:KStar. None | Some a
     Result = Lambda t:KStar. Lambda e:KStar. Ok t | Err e
   Type applications reduce like beta-reduction:
     Option Int ==> None | Some Int

   WELL-KINDED TYPES:
   A type is well-kinded if it has a valid kind in some context.
   The kinding judgment Delta |- T :: K ensures:
   - Type variables have their declared kind
   - Type applications have matching argument/parameter kinds
   - Proper types (values exist) have kind KStar

   RELATION TO F* TYPE SYSTEM:
   F* itself uses a similar kind system internally:
   - Type : Type is roughly our KStar : kind
   - Type -> Type corresponds to KArrow KStar KStar
   F*'s universe polymorphism generalizes this further.
   ============================================================================ *)
...
(* ============================================================================
   DEFAULT NAMED KIND ENVIRONMENT
   ============================================================================

   This environment provides kinds for standard library types, enabling
   kind inference for programs using common type constructors.

   ORGANIZATION BY KIND LEVEL (per Pierce TAPL Chapter 29):

   LEVEL 0 - Base Kinds (KRow, KRegion):
   - Effect labels: IO, Pure, Async, State, Error, etc.
   - Lifetime markers: Static, 'a, 'b, etc.

   LEVEL 1 - Simple Type Constructors (KStar -> KStar):
   - Containers: Option, Vec, List, Set
   - Smart pointers: Box, Rc, Arc
   - References: Ref, Cell, RefCell, Mutex
   - Async: Future, Promise, Stream

   LEVEL 2 - Binary Type Constructors (KStar -> KStar -> KStar):
   - Result, Either, Map, Pair

   LEVEL 3 - Higher-Kinded Constraints ((KStar -> KStar) -> KStar):
   - Functor, Applicative, Monad, Foldable, Traversable
   These take a TYPE CONSTRUCTOR and return a TYPE (the constraint).

   LEVEL 4 - Even Higher ((KStar -> KStar -> KStar) -> KStar):
   - Bifunctor: operates on binary type constructors

   WHY INCLUDE TYPECLASSES IN KIND ENV?

   The expression "Functor Option" needs kind checking:
   - Functor : (KStar -> KStar) -> KStar
   - Option  : KStar -> KStar
   - Functor Option : KStar (the constraint type)

   Without this entry, we couldn't kind-check typeclass constraints.

   SPEC COMPLIANCE:
   - Definition 3.5: Higher-Kinded Type syntax
   - Example 3.6: Functor as (Type -> Type) -> Type

   REFERENCES:
   - fstar_pop_book.md lines 12186-12357 (F* typeclasses)
   - Pierce TAPL Chapter 29.4 (higher-kinded polymorphism)
...
let default_named_kind_env : named_kind_env =
  [ (* ========================================================================
    ...
    (* ========================================================================
       ...
       ========================================================================
       These are "type classes" in the Haskell sense - type-level functions
       that take a type constructor and return a type (the "constraint" type).

       THEORETICAL BACKGROUND (per Pierce TAPL Chapter 29.4):
       Higher-kinded polymorphism allows type variables with non-* kinds.
       When F : * -> *, we can write functions like:
         fmap : forall F. Functor F => forall a b. (a -> b) -> F a -> F b

       The kind (KStar -> KStar) -> KStar means:
       - Takes a type constructor F : KStar -> KStar
       - Returns a proper type KStar (representing the "Functor F" constraint)

       WHY (KStar -> KStar) -> KStar FOR TYPECLASSES:
       1. The constraint "Functor F" must be a type (kind KStar)
       2. F must be a type constructor (kind KStar -> KStar)
       3. So Functor takes KStar -> KStar and returns KStar: (KStar -> KStar) -> KStar

       TYPECLASS HIERARCHY (supported here):
       - Functor F: provides fmap : (a -> b) -> F a -> F b
       - Applicative F: Functor + pure : a -> F a, (<*>) : F (a->b) -> F a -> F b
       - Monad F: Applicative + bind : F a -> (a -> F b) -> F b
       - Foldable F: fold : (b -> a -> b) -> b -> F a -> b
       - Traversable F: traverse : Applicative G => (a -> G b) -> F a -> G (F b)

       See fstar_pop_book.md lines 12186-12357 for F* typeclass implementation.
       ======================================================================== *)
    ...
    (* ========================================================================
       ...
       ========================================================================
       Bifunctor operates on binary type constructors like Either, Pair, or Result.
       Kind: (KStar -> KStar -> KStar) -> KStar  which is (KStar -> (KStar -> KStar)) -> KStar

       Bifunctor B provides:
         bimap : (a -> c) -> (b -> d) -> B a b -> B c d
         first : (a -> c) -> B a b -> B c b
         second : (b -> d) -> B a b -> B a d

       Common instances: Either, Pair, Result, (,), These
       ======================================================================== *)
...
(** ============================================================================
    VARIANCE IN HIGHER-KINDED TYPES (per TAPL Section 15.2)
    ============================================================================

    Variance describes how SUBTYPING propagates through parameterized types.
    This is crucial for type-safe polymorphism with generics.

    THEORETICAL FOUNDATION (Pierce TAPL Chapter 15):

    Given a type constructor F : * -> * and types T <: U (T is subtype of U):
    - COVARIANT (+): F<T> <: F<U>       (preserves direction)
    - CONTRAVARIANT (-): F<U> <: F<T>   (reverses direction)
    - INVARIANT (=): No relationship    (requires exact match)
    - BIVARIANT: Both hold              (unused/phantom parameter)

    STANDARD VARIANCE RULES:

    1. CONTAINER TYPES (producers) are typically COVARIANT:
       - Option<Dog> <: Option<Animal>      (can read Animals from Dog option)
       - Vec<Dog> <: Vec<Animal>            (can iterate Animals from Dog vec)
       - Iterator<Dog> <: Iterator<Animal>  (produces Dogs, which are Animals)

    2. FUNCTION PARAMETERS (consumers) are CONTRAVARIANT:
       - (Animal -> R) <: (Dog -> R)        (Dog handler works for any Animal)
       - Consumer<Animal> <: Consumer<Dog>  (Animal consumer accepts Dogs)

    3. MUTABLE REFERENCES are INVARIANT:
       - Cell<Dog> has NO relation to Cell<Animal>
       - Because: read gives Dog (covariant), write takes Dog (contravariant)
       - Combined: both variance, so invariant

    4. PHANTOM TYPES are BIVARIANT:
       - Parameter not used, so any relationship holds

    FUNCTOR/MONAD VARIANCE:

    For Functor F to be sound: F must be covariant
    - fmap : (a -> b) -> F a -> F b
    - If F covariant: fmap f preserves subtyping
    - Contravariant functors exist (Contravariant typeclass)
...
(* ============================================================================
   VARIANCE IMPLEMENTATION - Subtyping Through Type Constructors
   ============================================================================

   VARIANCE AS MONOTONICITY (Mathematical View):
   If we view subtyping as a partial order (<:), variance describes
   whether a type constructor F is monotonic or anti-monotonic:
   - Covariant:     F is monotonic (order-preserving)
   - Contravariant: F is anti-monotonic (order-reversing)
   - Invariant:     F is neither (only preserves equality)
   - Bivariant:     F is constant (ignores argument)

   CATEGORICAL INTERPRETATION:
   In category theory, types form a category with morphisms as
   coercions (subtype witnesses). Then:
   - Covariant functor: preserves morphism direction
   - Contravariant functor: reverses morphism direction
   This is why fmap for Functor requires covariance.

   ALGEBRAIC STRUCTURE OF VARIANCE:
   Variance forms a monoid under composition:
   - Identity: Covariant (Cov o V = V = V o Cov)
   - Composition: see combine_variance
   - The monoid is also commutative

   Additionally, {Covariant, Contravariant} forms a group (Z/2Z):
   - Identity: Covariant
   - Inverse: Contra^(-1) = Contra, Cov^(-1) = Cov
   - Contra o Contra = Cov (negation cancels)

   PRACTICAL IMPACT ON SUBTYPING:
   For F<G<T>> where F has variance v1 at G and G has variance v2 at T:
   - Overall variance = v1 o v2
   - Example: Fn<Option<T>, R> has variance Contra o Cov = Contra in T

   VARIANCE AND SOUNDNESS:
   Incorrect variance leads to type system unsoundness:
   - If Cell<T> were covariant, we could put Dog in Cell<Animal>,
     then read it as Animal, but write Cat - type error!
   - Java arrays are covariant but this is UNSOUND (throws at runtime)
   ============================================================================ *)
...
(* ============================================================================
   VARIANCE COMPOSITION - combine_variance
   ============================================================================
   ...
   ALGEBRAIC STRUCTURE (Pierce TAPL Section 15.2 extended):

   Variance composition forms a commutative monoid with absorption:

   MULTIPLICATION TABLE:
           | Cov   | Contra | Inv   | Biv
   --------+-------+--------+-------+------
   Cov     | Cov   | Contra | Inv   | Biv
   Contra  | Contra| Cov    | Inv   | Biv
   Inv     | Inv   | Inv    | Inv   | Biv
   Biv     | Biv   | Biv    | Biv   | Biv

   PROPERTIES PROVEN BELOW:
   1. Associativity:  (v1 o v2) o v3 = v1 o (v2 o v3)
   2. Commutativity:  v1 o v2 = v2 o v1
   3. Identity:       Cov o v = v
   4. Absorption:     Biv o v = Biv, Inv o v = Inv (when v not Biv)
   5. Self-inverse:   Contra o Contra = Cov

   INTUITION:
   - Bivariant: parameter unused, so variance doesn't matter
   - Invariant: both read and write, so any direction breaks
   - Covariant: "same direction" - doesn't change anything
   - Contravariant: "opposite direction" - flips once

   EXAMPLE - Function type (a -> b) -> (c -> d):
   - Outer function is contravariant in its parameter (a -> b)
   - Inner (a -> b) is contravariant in a, covariant in b
   - Overall: Contra o Contra = Cov in a, Contra o Cov = Contra in b
...
(** ============================================================================
    ...
    We represent this explicitly with:
    - tc_params: the bound type variables with their kinds
    ...
    In System F-omega (the basis for TAPL Chapter 29), we have:
      Types:  T ::= X | T -> T | forall X::K. T | Lambda X::K. T | T T'

    The last two forms (Lambda and application) are type operators.
    Type constructors are Lambda terms:
      Option : * -> * = Lambda X:*. Option_body(X)

    Applying a type constructor to an argument is type-level beta reduction:
      Option Int = Option_body[X := Int]

    WELL-FORMED TYPE CONSTRUCTORS:

    A type constructor is well-formed if:
    1. All type parameters are distinct (no shadowing)
    2. The body is well-kinded under the extended context
    3. Free type variables in body are bound by parameters
    4. Variance annotations are sound (match actual usage)

    KIND OF TYPE CONSTRUCTOR:

    The kind is determined by parameters:
      type_constructor_kind({params=[(a,k1),(b,k2)], ...}) = k1 -> k2 -> *

    This allows checking that type applications are well-kinded:
      Option<Int> : * because Option : * -> * and Int : *

    VARIANCE ANNOTATIONS:

    Each parameter has a variance that constrains how it can appear:
    - Covariant: can appear in positive positions (return types)
    - Contravariant: can appear in negative positions (parameters)
    - Invariant: can appear in both (mutable references)

    These are used for subtyping:
      Option<Dog> <: Option<Animal> because Option is covariant
      Ref<Dog> NOT <: Ref<Animal> because Ref is invariant

    REFERENCES:
    - Pierce TAPL Chapter 29 "Type Operators and Kinding"
    - Cardelli, "Type Systems" (handbook chapter)
...
(** ============================================================================
    ...
    In ordinary polymorphism, type variables range over TYPES (kind KStar):
      id : forall a:KStar. a -> a
    Here 'a' can be Int, String, Option<Bool>, etc.

    In HIGHER-KINDED polymorphism, type variables range over TYPE CONSTRUCTORS:
      fmap : forall F:(KStar->KStar). Functor F => forall a b. (a -> b) -> F a -> F b
    Here 'F' can be Option, List, Vec, Result<_, E>, etc.

    This is called "higher-kinded" because F has kind KStar -> KStar (higher than KStar).

    WHY HKT MATTERS - Pierce TAPL Chapter 29.4:

    HKT enables powerful abstractions that work uniformly over "shapes":

    1. FUNCTOR: Abstract over "mappable containers"
       - fmap transforms contents without changing structure
       - Works for Option, List, Vec, Tree, Future, Result, etc.

    2. MONAD: Abstract over "sequenceable computations"
       - bind chains dependent computations
       - Works for Option (null handling), Result (error handling),
         Future (async), State, Reader, Writer, etc.

    3. TRAVERSABLE: Abstract over "structures to iterate"
       - traverse visits each element with an effect
       - Works for List, Tree, Vec, etc.

    ENCODING IN TYPE SYSTEM:

    The constraint "Functor F" is itself a TYPE with kind:
      Functor : (KStar -> KStar) -> KStar

    A "Functor instance" is a VALUE of type (Functor F) - a dictionary
    containing the fmap implementation for that specific F.

    F* IMPLEMENTATION (fstar_pop_book.md lines 12186-12357):
    ```fstar
    ...
    instance functor_option : functor option = {
      fmap = fun #a #b f -> function
        | None -> None
        | Some x -> Some (f x)
    }
    ```

    FUNCTOR LAWS (must be proven for sound instances):
    1. fmap id = id                      (identity)
    2. fmap (f . g) = fmap f . fmap g    (composition)

    MONAD LAWS (must be proven for sound instances):
    ...
    - Marlow, "An Extensible Dynamically-Typed Hierarchy of Exceptions"
    - fstar_pop_book.md lines 12186-12357
