(**
 * BrrrLang.Core.SessionTypes - Implementation
 *
 ...
 * ---------------------------------------------------------------------------
 *
 * This module implements session types as introduced by Honda (1993):
 *   [Honda93] K. Honda. "Types for Dyadic Interaction." CONCUR 1993.
 *
 * Key theoretical extensions incorporated:
 *   - Labeled branching (Honda, Vasconcelos, Kubo 1998)
 *   - Subtyping (Gay & Hole 2005)
 *   - Priority-based deadlock freedom (Kobayashi 2006, Padovani 2014)
 *
 * IMPORTANT: Implements corrections from SPECIFICATION_ERRATA.md Chapter 5:
 *   - Uses association relation (Scalas & Yoshida 2019) instead of Honda's coherence
 *   - Uses subtyping in projection checking (Yoshida & Hou 2024)
 *
 * ---------------------------------------------------------------------------
 * GRAMMAR
 * ---------------------------------------------------------------------------
 ...
 * ---------------------------------------------------------------------------
 *
 * Duality swaps the perspective between channel endpoints:
 *
 *   dual(!t.S)       = ?t.dual(S)
 *   dual(?t.S)       = !t.dual(S)
 ...
 *   dual(end)        = end
 *
 * KEY THEOREM (Duality Involution - Theorem 5.1):
 *   For all session types S: dual(dual(S)) = S
 *
 * This is proven below in `dual_involutive` using structural induction.
 * The involution property is fundamental for communication safety.
 *
 * ---------------------------------------------------------------------------
 * SUBTYPING (Gay & Hole 2005)
 * ---------------------------------------------------------------------------
 ...
 *   !t.S1 <: !t.S1'  if S1 <: S1'
 *   ?t.S1 <: ?t.S1'  if S1 <: S1'
 *   +{...} <: +{...} (can offer more labels)
 *   &{...} <: &{...} (must handle fewer labels)
 *
 * The implementation uses coinductive (fuel-based) subtyping with
 * visited set to handle recursive types correctly.
 *
 * ---------------------------------------------------------------------------
 * PRIORITY-BASED DEADLOCK FREEDOM (Kobayashi 2006)
 * ---------------------------------------------------------------------------
 *
 * Well-typedness alone does NOT guarantee deadlock freedom.
 * This module includes priority annotations (pri_session) that enforce
 * strictly increasing priorities along sessions, preventing circular waits.
 *
...
(** ============================================================================
    ...
    THEORETICAL BACKGROUND (Honda 1993, Definition 5.2 in specification):

    Duality is THE fundamental operation in session types. It captures the
    symmetry of communication: what one party sends, the other receives.

    FORMAL DEFINITION:
      dual(!t.S)       = ?t.dual(S)         -- output becomes input
      dual(?t.S)       = !t.dual(S)         -- input becomes output
      dual(+{l_i:S_i}) = &{l_i:dual(S_i)}   -- internal choice becomes external
      dual(&{l_i:S_i}) = +{l_i:dual(S_i)}   -- external choice becomes internal
      dual(uX.S)       = uX.dual(S)         -- recursion distributes
      dual(X)          = X                  -- variables are self-dual
      dual(end)        = end                -- termination is self-dual

    KEY PROPERTY (Theorem 5.1 - Duality Involution):
      For all session types S: dual(dual(S)) = S

    This is proven below in `dual_involutive`.

    COMMUNICATION SAFETY CONSEQUENCE:
    If process P uses channel c at type S, and process Q uses c at type dual(S),
    then P and Q can communicate without type errors:
    - When P sends (type !t), Q receives (type ?t)
    - When P makes internal choice (+), Q offers external choice (&)

    REFERENCE:
    - Honda (1993), Section 2.3
    - Gay & Hole (2005), Definition 3.1 *)
...
(* Compute the dual of a session type.
   ...
   INVARIANTS:
   - session_size (dual s) = session_size s   (size is preserved)
   - free_session_vars (dual s) = free_session_vars s  (variables preserved)
   - is_wellformed s ==> is_wellformed (dual s)  (well-formedness preserved) *)
...
(** ============================================================================
    ...
    This section proves the FUNDAMENTAL THEOREM of session types:
    duality is an involution, i.e., dual(dual(S)) = S for all S.

    THEOREM 5.1 (Duality Involution):
      For all session types S: dual(dual(S)) = S

    PROOF STRATEGY:
    The proof proceeds by structural induction on S:
    - Base cases (SEnd, SVar): Trivial, as dual maps them to themselves.
    - Inductive cases: Apply IH to subterms and use equational reasoning.

    The proof is fully mechanized below, providing machine-checked
    verification of this core property.

    REFERENCES:
    - Honda (1993), Proposition 2.4
    - Gay & Hole (2005), Lemma 3.4
    - Brrr-Lang Specification, Theorem 5.1 *)

(* Lemma: dual_action is an involution.
   Base case for multi-channel select duality. *)
...
(** ============================================================================
    ...
    S1 <: S2 means a channel of type S1 can be used wherever S2 is expected.

    SUBTYPING RULES (coinductive definition):

      !t.S1 <: !t'.S1'   if t <: t' and S1 <: S1'
        (Send: covariant in payload, covariant in continuation)

      ?t.S1 <: ?t'.S1'   if t' <: t and S1 <: S1'
        (Receive: CONTRAVARIANT in payload, covariant in continuation)

      +{l_i:S_i}_{i in I} <: +{l_j:S_j'}_{j in J}  if I subseteq J and
        for all i in I: S_i <: S_i'
        (Internal choice: can offer FEWER labels)

      &{l_i:S_i}_{i in I} <: &{l_j:S_j'}_{j in J}  if J subseteq I and
        for all j in J: S_j <: S_j'
        (External choice: must handle ALL required labels)

    INTUITION:
    - Send: You can send a MORE specific (subtype) value
    - Receive: You can receive a LESS specific (supertype) value
    - Internal choice (+): A client that selects fewer labels is compatible
    - External choice (&): A server that handles more labels is compatible

    COINDUCTIVE ALGORITHM (handles recursive types):

    For recursive types like uX.!int.X, we need coinductive reasoning:
    1. Maintain a "visited set" of pairs (S1, S2) assumed to be subtypes
    2. When we revisit a pair, return true (coinductive hypothesis)
    3. Use fuel for termination guarantee

    SOUNDNESS: This computes an underapproximation of the true coinductive
    subtyping relation. If session_subtype_co returns true, the types ARE
    definitely subtypes. If false, they may or may not be (fuel exhaustion).

    IMPORTANT (Honda 2008 correction - see SPECIFICATION_ERRATA.md):
    The original Honda formulation used EQUALITY in projection checking.
    ...
    REFERENCES:
    - Gay & Hole (2005), Section 4 "Subtyping Algorithm"
    - Scalas & Yoshida (2019), Section 3 "Association Relation"
*)
