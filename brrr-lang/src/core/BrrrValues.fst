(**
 * BrrrLang.Core.Values - Implementation
 *
 ...
 *
 *   - Environment operations: lookup, extend, remove
 *   - Heap operations: alloc, read, write, dealloc
 *   - Result monad: return, bind, map_result
 *   - State monad: st_return, st_bind, get_env, set_env, etc.
 *   - Value operations: lit_to_value, value_eq, value_compare
 *   - Pattern matching: match_pattern, match_patterns
 *   - Type inference: type_of_value
 *   - Arithmetic: int_add, int_sub, int_mul, int_div
 *
 * == Key Design Decisions ==
 *
 * 1. Association Lists for Environment/Heap:
 *    Both env and heap are implemented as association lists [(key, value), ...].
 *    This provides O(n) lookup but simple reasoning in proofs.
 *    For production, these would be replaced with hash maps.
 *
 * 2. IEEE 754 Float Semantics:
 *    value_eq uses IEEE 754 semantics (NaN != NaN).
 *    For reflexive equality (needed in proofs), use value_eq_bits.
 *
 * 3. Pattern Matching Limitations:
 *    match_pattern is a pure function without heap access.
 *    PatRef and PatBox patterns match the wrapper, not dereferenced content.
 *    PatGuard and PatType patterns require evaluation context (see BrrrEval.fst).
 *
 * 4. Type Inference vs Type Checking:
 *    type_of_value infers types from runtime values (bottom-up).
 *    Some values (VNone, VRef, VClosure) lack sufficient type info,
 *    returning TPrim PUnknown. The evaluator tracks actual types externally.
 *
 * == Proof Patterns ==
 *
 * This file uses several F* proof patterns:
 *
 *   - #push-options / #pop-options for localized solver settings
 *   - [SMTPat ...] for automatic lemma instantiation
 *   - Mutual recursion with termination measures (value_size, pattern_size)
 *   - Classical reasoning for value equality properties
 *
 * == Known Issues (SPECIFICATION_ERRATA.md Chapter 7) ==
 *
 * The match_pattern_patvar lemma uses admit() due to a type mismatch:
 * match_pattern takes `pattern` (= with_loc pattern') but matches against
 * pattern' constructors. This is documented in the errata and the lemma
 * holds semantically.
 *
 * == References ==
 *
 ...
 * - HACL* Lib.IntTypes: Integer type patterns
...
(** ============================================================================
    ...
    The environment is a mapping from variable identifiers to values.
    Implemented as an association list for simplicity in proofs.

    Design Notes:
    - New bindings are prepended (shadowing semantics)
    - Lookup finds the first (most recent) binding
    - Remove filters out ALL bindings for a variable

    Time Complexity: O(n) for all operations
    Space Complexity: O(n) for the list

    For production use, these would be replaced with hash maps,
    but association lists simplify F* reasoning about variable bindings.

    See: brrr_lang_spec_v0.4.tex Section 3.1 (Environments)
    ============================================================================ *)

(** Empty environment: the initial state with no bindings *)
let empty_env : env = []

(** Lookup variable in environment.
    Returns the value bound to the most recent (innermost) binding of x.
    Returns None if x is not bound.

    Note: Uses List.Tot.assoc which returns the first matching entry. *)
let lookup (x: var_id) (e: env) : option value =
  List.Tot.assoc x e

(** Extend environment with a new binding.
    The new binding shadows any existing binding for x.

    Invariant: lookup x (extend x v e) == Some v *)
let extend (x: var_id) (v: value) (e: env) : env =
  (x, v) :: e

(** Extend environment with multiple bindings.
    Bindings are added in order: earlier in list = inner scope.

    Example: extend_many [(x, v1), (y, v2)] e
    is equivalent to: extend y v2 (extend x v1 e)
    but note the list is prepended, so y shadows x if both are same. *)
let extend_many (bindings: list (var_id & value)) (e: env) : env =
  bindings @ e

(** extend_many with singleton list equals extend.
    This lemma is used in let-binding proofs where we have
    a single pattern binding that produces one variable. *)
...
(** Remove variable from environment.
    Removes ALL bindings for x, not just the most recent one.

    Note: This is needed for let-rec where we need to remove
    the old binding before adding the recursive one. *)
let remove (x: var_id) (e: env) : env =
  List.Tot.filter (fun (y, _) -> y <> x) e

(** Domain of environment: list of bound variable identifiers.
    May contain duplicates if a variable is shadowed. *)
...
(** ============================================================================
    ...
    The heap maps locations (natural numbers) to values.
    Implements a simple store model for mutable state.

    From brrr_lang_spec_v0.4.tex:
      Heap : Loc -> Tagged[Val]
      where Tagged[Val] includes runtime type information

    Design Notes:
    - Locations are natural numbers (non-negative)
    - Location 0 is never allocated (sentinel)
    - next_loc returns 1 + max(all locations in heap)
    - alloc always returns a fresh location

    Memory Model:
    - alloc: Allocates new cell, returns fresh location
    - read: Returns value at location (None if unallocated)
    - write: Updates value at location (or creates if new)
    - dealloc: Frees location (subsequent reads return None)

    Invariants (proven in lemmas below):
    - alloc returns location not previously in heap
    - write l v h; read l h' == Some v
    - dealloc l h; read l h' == None

    See: brrr_lang_spec_v0.4.tex Section 3.1 (Heap)
    ============================================================================ *)

(** Empty heap: the initial state with no allocations *)
let empty_heap : heap = []

(** Compute next available location.
    Returns 1 + maximum location currently in use.

    Invariant: next_loc h > l for all (l, _) in h
    This ensures alloc always returns a fresh location.

    Note: Returns 1 for empty heap (location 0 is reserved). *)
let next_loc (h: heap) : loc =
  1 + List.Tot.fold_left (fun m (l, _) -> max m l) 0 h

(** Allocate value on heap.
    Returns (new_location, updated_heap).

    Post-conditions (proven in alloc_fresh):
    - read new_location old_heap == None  (location was fresh)
    - read new_location new_heap == Some v (value is stored) *)
let alloc (v: value) (h: heap) : loc & heap =
  let l = next_loc h in
  (l, (l, v) :: h)

(** Read value at location.
    Returns Some v if location is allocated, None otherwise.

    Note: Multiple entries for same location are possible (after write).
    Returns the first (most recent) entry. *)
let read (l: loc) (h: heap) : option value =
  List.Tot.assoc l h

(** Write value at location.
    Creates or updates the binding at location l.

    Implementation: prepends new binding and filters out old ones.
    This ensures read l (write l v h) == Some v.

    Note: Does NOT check if location was previously allocated.
    Writing to unallocated location effectively allocates it. *)
let write (l: loc) (v: value) (h: heap) : heap =
  (l, v) :: List.Tot.filter (fun (l', _) -> l' <> l) h

(** Deallocate location.
    Removes all bindings for location l.

    After dealloc: read l (dealloc l h) == None

    Note: Deallocating an unallocated location is a no-op.
    This is safe but may indicate a bug (double-free). *)
let dealloc (l: loc) (h: heap) : heap =
  List.Tot.filter (fun (l', _) -> l' <> l) h

(** ============================================================================
    COMPUTATION RESULTS (RESULT MONAD)
    ============================================================================

    The result type forms a monad with return and bind operations.
    This implements the Comp[A] type from the specification:

      Comp[A] = Ok(A) + Err(Val) + Div + ControlEffects

    == Monad Laws ==

    The monad laws are proven in the RESULT MONAD LAWS section:
    ...
    The bind operation implements short-circuit evaluation:
    - ROk x: continues with f x
    - All other constructors: propagate unchanged

    This gives us exception-like semantics where errors/control flow
    propagate up the call stack without executing subsequent code.

    == Usage Pattern ==

    The let* syntax provides monadic do-notation:

      let* v1 = eval e1 in
      let* v2 = eval e2 in
      return (combine v1 v2)

    If e1 evaluates to RErr/RBreak/etc, the error propagates immediately.

    See: brrr_lang_spec_v0.4.tex Section 3.2 (Computation Monad)
    ============================================================================ *)

(** Monadic return: wrap a value in ROk.
    This is the unit of the result monad. *)
let return (#a: Type) (x: a) : result a = ROk x

(** Monadic bind: sequence computations with short-circuit semantics.

    If m is ROk x, applies f to x and returns the result.
    If m is any other constructor (error/control), propagates it unchanged.

    This is the core operation that gives us exception-like behavior.

    Note: Fuel 1 needed for exhaustive pattern matching on noeq type. *)
...
(** let* syntax for monadic do-notation.
    Allows writing: let* x = m in body
    instead of: bind m (fun x -> body) *)
let (let*) = bind

(** Functor map: apply pure function to result value.
    Equivalent to: bind r (fun x -> return (f x))
    but implemented directly for efficiency.

    ROk x is mapped to ROk (f x).
    All other constructors propagate unchanged. *)
#push-options "--fuel 1 --ifuel 1"
...
(** Extract value from ROk result.
    Requires ROk? r as precondition (refinement type).

    Note: This is an explicit projector because result is noeq,
    which means F* doesn't automatically generate projectors. *)
...
let lit_to_value (l: literal) : value =
  ...
  | LitChar c -> VChar c
  | LitImaginary f prec ->
      (* Imaginary literal: fi -> complex(0, f) = VTuple [VFloat 0 prec; VFloat f prec] *)
      VTuple [VFloat 0 prec; VFloat f prec]
...
(** ============================================================================
    ...
    Pattern matching matches values against patterns to produce bindings.

    == KNOWN ISSUE (SPECIFICATION_ERRATA.md Chapter 7) ==

    CRITICAL: This code has a type mismatch that needs fixing.

    The type `pattern = with_loc pattern'` wraps pattern' with location info.
    The pattern match should be:

        match p.loc_value with    (* Extract the pattern' from with_loc *)
        | PatWild -> ...

    NOT:

        match p with              (* p is with_loc pattern', not pattern' *)
        | PatWild -> ...

    The current code matches directly on `p`, which is `with_loc pattern'`,
    but the constructors (PatWild, PatVar, etc.) are for `pattern'`.

    This causes F* Error 114: "Type of pattern does not match type of scrutinee"

    TODO: Fix by changing all `match p with` to `match p.loc_value with`
    in pattern_size, pattern_list_size, field_pattern_list_size,
    match_pattern, match_patterns, and match_struct_fields.

    For now, this code may fail typechecking until the fix is applied.

    See: SPECIFICATION_ERRATA.md Chapter 7 for full details.
    ============================================================================ *)

(* Pattern size functions for termination measure.
   NOTE: See KNOWN ISSUE above - needs p.loc_value extraction. *)
...
let rec match_pattern (p: pattern) (v: value)
  ...
  (* DESIGN LIMITATION: PatRef and PatBox matching cannot dereference
     ...
     For proper reference dereferencing, use eval_match_pattern in BrrrEval.fst
     ...
     in BrrrValues.fst is a pure operation, while heap access is effectful.
  ...
  *)
  ...
  | PatGuard _ _ -> None  (* Guards need evaluation context - see BrrrEval.fst *)
  ...
  (* PatType: Type pattern - matches if value's runtime type matches expected type.
     ...
     The full implementation is in eval_match_pattern in BrrrEval.fst.
  ...
  | PatType _ _ -> None  (* Type patterns need evaluation context - see BrrrEval.fst *)
...
let lit_to_value_imaginary (f: float_repr) (prec: float_prec)
