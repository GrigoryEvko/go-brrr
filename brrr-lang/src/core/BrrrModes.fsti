 ...
 * ============================================================================
 *
 * This module implements a MODE SEMIRING for tracking resource usage in
 * substructural type systems. The theory originates from Girard's Linear Logic
 * (1987) and has been extensively developed for practical type systems.
 *
 * KEY REFERENCES:
 *   [Girard 1987] Jean-Yves Girard. "Linear Logic." Theoretical Computer
 *                 Science, 50(1):1-101, 1987.
 *                 - Foundational paper on linear logic and resource semantics
 *                 - Introduces exponential modalities (!) for non-linear types
 *
 *   [Walker 2005] David Walker. "Substructural Type Systems." Chapter in
 *                 Advanced Topics in Types and Programming Languages, MIT Press.
 *                 - Comprehensive treatment of linear, affine, and relevant types
 *                 - Mode semiring formalization (Section 1.3)
 *
 *   [Abel & Bernardy 2020] Andreas Abel, Jean-Philippe Bernardy.
 *                          "A Unified View of Modalities in Type Systems."
 *                          ICFP 2020.
 *                          - Modern treatment unifying coeffect and effect systems
 *
 *   [Atkey 2018] Robert Atkey. "Syntax and Semantics of Quantitative Type Theory."
 *                LICS 2018.
 *                - Quantitative type theory with semiring-indexed types
 *
 * ============================================================================
 * MODE SEMIRING STRUCTURE
 * ============================================================================
 *
 * The mode semiring is the algebraic structure (M, +, *, 0, 1) where:
 *
 *   M = {0, 1, omega}
 *
 * Elements:
 *   - 0 (MZero)  : absent/unavailable - the variable cannot be used
 *   - 1 (MOne)   : linear - the variable must be used exactly once
 *   - omega (MOmega) : unrestricted - the variable can be used any number of times
 *
 * Operations:
 *   (+) : Mode addition represents PARALLEL composition of usages
 *         When two subexpressions both use a variable, their modes add.
 *         This is used in the L-App rule for function application.
 *
 *         Addition table:
 *           0 + m = m        (identity)
 ...
 *           omega + omega = omega
 *
 *   [*] : Mode multiplication represents SEQUENTIAL composition
 *         When a variable is used through a binding, modes multiply.
 *         This is used in the L-Let rule for let expressions.
 *
 ...
 *           omega * omega = omega
 *
 * The semiring satisfies:
 *   - Additive identity: 0 + m = m
 *   - Additive commutativity: m1 + m2 = m2 + m1
 *   - Additive associativity: (m1 + m2) + m3 = m1 + (m2 + m3)
 *   - Multiplicative identity: 1 * m = m
 *   - Multiplicative annihilation: 0 * m = 0
 *   - Distributivity: m1 * (m2 + m3) = (m1 * m2) + (m1 * m3)
 *
 * ============================================================================
 * EXTENDED MODES (Substructural Type Systems)
 * ============================================================================
 *
 * Beyond the basic semiring, we track EXTENDED MODES that determine which
 * structural rules are permitted:
 *
 *   EMLinear      : No weakening, no contraction - use EXACTLY once
 *   EMAffine      : Weakening allowed, no contraction - use AT MOST once
 *   EMRelevant    : No weakening, contraction allowed - use AT LEAST once
 *   EMUnrestricted: Both allowed - use ANY number of times
 *
 * This forms a lattice (see SPECIFICATION_ERRATA.md, Chapter 2):
 *
 ...
 *
 * Structural rules:
 *   - Weakening: Gamma, x:A |- t : B  implies  Gamma |- t : B  (drop unused)
 *   - Contraction: Gamma, x:A, y:A |- t : B  implies  Gamma, z:A |- t[z/x,z/y] : B
 *
 * Connection to Rust ownership:
 *   - Owned + !Copy : EMLinear (must move exactly once)
 *   - Owned + Copy  : EMUnrestricted (can copy freely)
 *   - &T (shared borrow) : EMUnrestricted (can share freely)
 *   - &mut T (exclusive borrow) : EMAffine (can drop but not duplicate)
 *
 * ============================================================================
 * CONTEXT SPLITTING (L-App Rule)
 * ============================================================================
 *
 * For typing function application (e1 e2), we need to split the linear context:
 *
 *        Gamma1 |- e1 : A -> B    Gamma2 |- e2 : A
 *        --------------------------------------------- (L-App)
 *                   Gamma1 + Gamma2 |- e1 e2 : B
 *
 * The split_ctx operation implements this by distributing modes:
 *   - MOmega variables: copied to BOTH sides (omega + omega = omega)
 *   - MOne variables: go to LEFT side only (1 + 0 = 1)
 *   - MZero variables: copied to BOTH sides (0 + 0 = 0)
 *
 * CRITICAL INVARIANT: Linear Exclusivity
 *   For contexts from a split, linear (EMLinear) resources have mode MOne
 *   in at most one context. This ensures join_ctx preserves validity:
 *   - mode_join MOne MZero = MOne (valid for EMLinear)
 *   - mode_join MZero MOne = MOne (valid for EMLinear)
 *   - mode_join MOne MOne = MOne (would be invalid if both had MOne)
 *
 *   See SPECIFICATION_ERRATA.md Chapter 2 for the detailed proof.
 *
 * ============================================================================
 * FRACTIONAL PERMISSIONS (Chapter 9)
 * ============================================================================
 *
 * Fractional permissions p in (0,1] enable READ SHARING:
 *   - p = 1 : full/exclusive ownership (can read AND write)
 *   - 0 < p < 1 : shared read-only access
 *
 * Key property: fractions SUM to at most 1
 *   - split: p -> (p/2, p/2)
 *   - join: (p1, p2) -> p1 + p2 (requires p1 + p2 <= 1)
 *
 * This enables the FREEZE/THAW pattern:
 *   - freeze: diamond_tau @ 1 -> box_tau @ omega (exclusive to shared)
 *   - thaw: box_tau @ full -> diamond_tau @ 1 (recover exclusive when all shares returned)
 *
 * Reference: Boyland's fractional permissions (2003) and Bornat et al. (2005).
 *
 * ============================================================================
 * IMPLEMENTATION NOTES
 * ============================================================================
 *
 * 1. All mode operations use 'unfold' for automatic normalization in proofs.
 *    This follows HACL* Lib.IntTypes patterns where operations normalize
 *    during type-checking, making most proofs trivial by reflexivity.
 *
 * 2. Lemmas have SMTPat triggers for automatic application during verification.
 *
 * 3. The join_preserves_valid theorem requires linear_exclusive precondition
 *    to prevent omega from appearing for linear resources (see errata).
 *)
module BrrrModes
...
(** Linear Exclusivity Predicate
    ...
    in at most one context. This ensures join won't produce omega for linear resources.
...
(* Join preserves validity ONLY when linear exclusivity holds.
   ...
   mode omega for a linear resource (1 + 1 = omega), violating validity. *)
