 ...
 * ============================================================================
 *
 * This module follows patterns from KaRaMeL (Karamel compiler for Low-star to C):
 *
 * - Type sizes are computed statically and match C sizeof()
 * - Struct layouts follow C ABI with proper padding
 * - Function signatures use C calling conventions
 * - Memory safety is verified at F-star level, erased at extraction
 *
 * The layout computation functions (compute_c_layout, compute_struct_size)
 * produce layouts identical to what a C compiler would generate with
 * default (non-packed) alignment settings.
 *
 * ============================================================================
 * CROSS-REFERENCES
 * ============================================================================
 *
 * - brrr_lang_spec_v0.4.tex: Part X (FFI & Interop)
 * - FSTAR_REFERENCE.md: Section 9 (Extraction), Section 11 (Low-star)
 * - synthesis_combined.md: Section 9.4 (FFI Contracts)
 * - HACL-star: Lib.IntTypes for integer type patterns
 * - EverParse: LowParse for serialization patterns
 *
 * ============================================================================
 * MODULE CONTENTS
 * ============================================================================
...
(* ============================================================================
    ...
    Visibility modifiers control which functions are exported across the FFI
    boundary and which remain internal. This is critical for:

    1. SYMBOL TABLE MANAGEMENT
       - FFIPublic functions are exported with #[no_mangle] to prevent
         name mangling, ensuring C code can link to them by name
       - FFIPrivate functions are not exported, preventing accidental
         calls from foreign code

    2. LINKER VISIBILITY
       - FFIPublic: "default" visibility - exported from shared library
       - FFIInternal: "hidden" visibility - visible within shared object only
       - FFIPrivate: "hidden" + not in export list

    3. API SURFACE CONTROL
       - Only FFIPublic functions form the public FFI API
       - Internal helpers can be FFIInternal for crate-level access
       - Implementation details should be FFIPrivate

    This follows Rust's visibility model (pub, pub(crate), private) adapted
    for C FFI contexts where symbol visibility matters for dynamic linking.

    KARAMEL MAPPING:
    - FFIPublic  -> C function with KRML_HOST_VISIBILITY_DEFAULT
    - FFIPrivate -> static C function (file scope only)
    - FFIInternal -> C function with KRML_HOST_VISIBILITY_HIDDEN
    ============================================================================ *)
...
(** Visibility modifiers control FFI export/import behavior.
    ...
    FFIInternal: Visible within crate/package but not exported to BrrrFFI.
...
(* ============================================================================
    ...
    Calling conventions define the low-level protocol for function calls:

    1. HOW ARGUMENTS ARE PASSED
       - Which CPU registers hold arguments (rdi, rsi, rcx, rdx on x64 System V)
       - Stack layout for overflow arguments
       - Struct passing conventions (by value, by reference, split across regs)

    2. RETURN VALUE HANDLING
       - Register(s) for return value (rax, xmm0 for floats)
       - Large struct return (hidden first parameter or sret)

    3. STACK MANAGEMENT
       - Who cleans up the stack (caller in cdecl, callee in stdcall)
       - Stack alignment requirements (16-byte on x64)

    4. REGISTER PRESERVATION
       - Which registers must be saved by callee (callee-saved/non-volatile)
       - Which registers may be clobbered (caller-saved/volatile)

    SPECIFICATION REFERENCE (brrr_lang_spec_v0.4.tex):
      "The calling convention (C, Rust, System) determines ABI."

    PLATFORM NOTES:
    - x86_64 Linux/macOS: System V AMD64 ABI (6 int regs, 8 float regs)
    - x86_64 Windows: Microsoft x64 (4 regs, shadow space required)
    - ARM64: AAPCS64 (8 int regs, 8 float regs)

    WARNING: Mixing calling conventions causes undefined behavior!
    Always use CC_C for maximum compatibility with C libraries.
    ============================================================================ *)

(** Supported calling conventions for BrrrFFI.
    ...
    CC_C:        Standard C calling convention (cdecl on x86, System V on x64).
                 ...
                 C library interop. Caller cleans up the stack on x86.

    CC_Rust:     Rust ABI - implementation-defined and unstable.
                 Only use for Rust-to-Rust FFI where both sides use
                 the same Rust compiler version. NOT recommended for
                 cross-language interop.

    CC_System:   Platform default convention.
                 - Windows x86: stdcall (callee cleans stack)
                 - Windows x64: Microsoft x64 ABI
                 - Unix: Same as CC_C
                 Use for Windows API interop (kernel32, user32, etc.).

    CC_Fastcall: Fastcall convention - first arguments in registers.
                 - x86: ecx, edx for first two int args
                 - x64: Not meaningfully different from standard x64 ABI
                 Rarely needed; prefer CC_C for portability. *)
...
(* ============================================================================
   ...
   From brrr_lang_spec_v0.4.tex Section "FFI-Safe Type Set":

     FFISafe ::= Int[w, s] for w in {8, 16, 32, 64}
             |   Float[p] for p in {32, 64}
             |   Ptr[T] for T in FFISafe
             |   FnPtr[T1, ..., Tn -> R]
             |   CStr
             |   Struct[f1:T1, ..., fn:Tn] with repr(C)

   SAFETY RATIONALE:

   1. WHY FIXED-WIDTH INTEGERS ONLY?
      - i128: Not uniformly supported; may use software emulation
      - IBig: Heap-allocated with GC, no stable C representation
      - C 'int': Platform-dependent size (16/32 bits historically)
      Explicit widths (i32, i64) ensure identical representation on all platforms.

   2. WHY F16 IS NOT FFI-SAFE?
      - Limited hardware support (_Float16 not in C standard)
      - Compiler/platform dependent emulation
      - Use f32 or explicit conversion instead

   3. POINTER SAFETY
      - Raw pointers (Ptr[T]) are FFI-safe when T is FFI-safe
      - The type system does NOT track pointer validity - that is the
        programmer's responsibility in unsafe blocks
      - FFIOpaquePtr (void pointer) accepts any pointer but provides no type info

   4. STRUCT REPRESENTATION
      - Only structs with repr(C) are FFI-safe
      - Rust-style structs have no guaranteed layout (compiler may reorder)
      - repr(C) guarantees C-compatible field ordering and padding

   MEMORY LAYOUT GUARANTEES:

   For each FFI-safe type, the module provides:
   - ffi_type_size: Exact byte size matching sizeof() in C
   - ffi_type_align: Alignment requirement matching _Alignof() in C
   - Proofs that all alignments are powers of 2

   CROSS-REFERENCE: HACL-star Lib.IntTypes uses similar fixed-width approach
   to ensure portable behavior across platforms.
   ============================================================================ *)
...
(** Types that are safe to pass across FFI boundaries.
    ...
    FFIInt:       Fixed-width integers with explicit width and signedness.
                  Maps directly to C int8_t, int16_t, int32_t, int64_t
                  and their unsigned variants.

    FFIFloat:     IEEE 754 floating point. F32 maps to C float,
                  F64 maps to C double. Both have identical representation
                  on all platforms following IEEE 754.

    FFIPtr:       Pointer to FFI-safe type. Maps to T* in C.
                  The pointed-to type must also be FFI-safe.
                  Size is platform-dependent (4 bytes on 32-bit, 8 on 64-bit).

    FFIFnPtr:     Function pointer with parameter and return types.
                  Maps to C function pointer: R (fn_ptr)(T1, ..., Tn).
                  Only unsafe functions can be passed as function pointers.

    FFICStr:      C-style null-terminated string: const char pointer.
                  The string data must be valid UTF-8 or ASCII.
                  Caller is responsible for null termination.

    FFIVoid:      Void type - only valid as return type (void function)
                  or pointer target (void pointer = opaque pointer).
                  Size is 0, but pointer to void has normal pointer size.

    FFIStruct:    C struct with named fields. Fields are laid out
                  according to C ABI with proper padding for alignment.
                  Only structs marked with #[repr(C)] can be converted.

    FFIOpaquePtr: Opaque pointer (void-star). Accepts any pointer type.
                  Use when the concrete type is unknown or irrelevant.
                  No type-safe operations available. *)
...
(* ============================================================================
    ...
    This section defines type sizes and alignments that match C ABI behavior.
    Correct layout computation is critical for FFI safety because:

    1. STRUCT FIELD ACCESS
       If offsets are wrong, reading a field reads garbage or crashes.
       The C compiler and Brrr must agree on every field's position.

    2. ALIGNMENT REQUIREMENTS
       Misaligned access causes SIGBUS on strict architectures (SPARC, older ARM).
       Even on x86, misaligned access is slower and non-atomic.

    3. STACK FRAME LAYOUT
       Stack-passed arguments must be at correct offsets.
       Stack alignment (typically 16 bytes on x64) must be maintained.

    DATA MODEL REFERENCE:

    +----------+--------+-------+------+------+----------+-------+---------+
    | Model    | char   | short | int  | long | long long| ptr   | Platforms|
    +----------+--------+-------+------+------+----------+-------+---------+
    | ILP32    | 8      | 16    | 32   | 32   | 64       | 32    | x86 Linux|
    | LP64     | 8      | 16    | 32   | 64   | 64       | 64    | x64 Linux|
    | LLP64    | 8      | 16    | 32   | 32   | 64       | 64    | x64 Win  |
    +----------+--------+-------+------+------+----------+-------+---------+

    KEY INSIGHT: Using explicit widths (i32 instead of 'int') avoids all
    these platform differences. This module uses fixed-width types for
    FFI-safe types precisely for this reason.

    KARAMEL REFERENCE: This approach matches KaRaMeL's extraction where
    F-star integers are mapped to C fixed-width types (int32_t, etc.)
    rather than platform-dependent types.
    ============================================================================ *)

(** Platform configuration for memory layout calculations.

    This record captures the platform-dependent aspects of memory layout
    without requiring the full target_abi type. It enables quick layout
    calculations when the full ABI is not available.
    ...
    Fields:
    - pc_pointer_size:  Pointer width in bytes (4 for 32-bit, 8 for 64-bit)
    - pc_pointer_align: Pointer alignment (usually equals pointer size)
    - pc_long_size:     C 'long' size (4 on LLP64/ILP32, 8 on LP64)
    - pc_size_t_size:   size_t size (usually equals pointer size) *)
...
(* ============================================================================
    ...
    Implements the C ABI struct layout algorithm.

    C STRUCT LAYOUT ALGORITHM:

    1. Fields are placed in declaration order (unlike Rust which may reorder)
    2. Each field is aligned to its natural alignment requirement
    3. Padding bytes are inserted between fields to satisfy alignment
    4. Total struct size is rounded up to struct alignment (max field align)

    EXAMPLE: struct { int8_t a; int32_t b; int8_t c; }

        Offset  Size  Field
        ------  ----  -----
        0       1     a (i8)
        1-3     3     <padding for b's 4-byte alignment>
        4       4     b (i32)
        8       1     c (i8)
        9-11    3     <padding for struct's 4-byte alignment>
    ...
    With #pragma pack(1) or __attribute__((packed)), no padding is inserted:
        Total: 6 bytes, aligned to 1

    WARNING: Packed structs may cause unaligned access faults on some platforms!

    KARAMEL REFERENCE: This algorithm matches KaRaMeL's struct layout for
    Low-star buffers and records when extracted to C.
    ============================================================================ *)

(** Field layout information: name, byte offset from struct start, size.

    This record captures the computed layout of a single struct field
    after C ABI padding rules have been applied. *)
type field_layout = {
  fl_name   : string;   (* Field name for diagnostics and offsetof generation *)
  fl_offset : nat;      (* Byte offset from struct start (after padding) *)
  fl_size   : nat       (* Field size in bytes *)
}
...
(* ============================================================================
    ...
    This section implements the FFI-safe type checking described in
    brrr_lang_spec_v0.4.tex:

        "T-Extern:
         forall i. T_i in FFISafe     sigma in FFISafe
         -------------------------------------------------
         env |- extern f : (T1, ..., Tn) -> sigma ; [FFI]"

    A type is FFI-safe if and only if it can be converted to an ffi_safe_type.
    This check is performed at compile time, not runtime.

    DESIGN PHILOSOPHY - DETAILED ERROR MESSAGES:

    Rather than returning a simple boolean, check_ffi_safe returns detailed
    error information explaining exactly WHY a type is not FFI-safe. This is
    critical for usability because FFI safety failures are common and often
    non-obvious to developers.

    ERROR CATEGORIES:

    1. REPRESENTATION ERRORS
       - FFISafeErrNeverType: The ! (never) type has no representation
       - FFISafeErrDynamicType: Dynamic (dyn) has no fixed layout
       - FFISafeErrBigInt: BigInt requires GC, cannot pass to C

    2. STRUCT REPRESENTATION ERRORS
       - FFISafeErrNonCRepr: Struct uses Rust repr (may reorder fields)
       - Must use #[repr(C)] for FFI structs

    3. FUNCTION POINTER ERRORS
       - FFISafeErrNonUnsafeFunction: Only unsafe functions can be passed
         as function pointers (safe functions may have hidden effects)

    4. WRAPPER TYPE ERRORS
       - FFISafeErrUnsupportedWrapper: Rc, Arc, Option, Mut wrappers
         have Rust-specific runtime requirements, not C-compatible

    5. NESTED ERRORS (for diagnostic chaining)
       - FFISafeErrNestedField: A struct field is not FFI-safe
       - FFISafeErrNestedParam: A function parameter is not FFI-safe
       - FFISafeErrNestedReturn: A function return type is not FFI-safe

    SPECIFICATION REFERENCE:
    From brrr_lang_spec_v0.4.tex Section "Safety and Unsafety":
        "FFI calls require unsafe context... Inside unsafe, the programmer
         asserts memory safety invariants."
    ============================================================================ *)

(** Detailed error type for FFI safety checking.

    Provides specific information about why a type is not FFI-safe.
    These errors are intended for developer-facing diagnostics and
    should include actionable fix suggestions where possible. *)
type ffi_safety_error =
  | FFISafeErrNeverType
      (* Never type has no representation - this type is uninhabited *)
  | FFISafeErrDynamicType
      (* Dynamic type has no fixed representation - size unknown at compile time *)
  | FFISafeErrBigInt
      (* BigInt is heap-allocated with GC, not C-compatible; use fixed-width instead *)
  | FFISafeErrNonCRepr : repr:repr_attr -> ffi_safety_error
      (* Struct has non-C representation; add #[repr(C)] attribute *)
  | FFISafeErrNonUnsafeFunction
      (* Only unsafe functions can cross FFI boundary as function pointers *)
  ...
  | FFISafeErrNestedField : struct_name:string -> field_name:string ->
      ...
      (* Field type in struct is not FFI-safe - includes field path for diagnostics *)
  | FFISafeErrNestedParam : param_idx:nat -> inner_error:ffi_safety_error -> ffi_safety_error
      (* Function parameter type is not FFI-safe - includes parameter index *)
  ...
  | FFISafeErrUnsupportedType : type_desc:string -> ffi_safety_error
      (* Catch-all for types without FFI representation (effects, capabilities, etc.) *)
...
(* ============================================================================
    ...
    From brrr_lang_spec_v0.4.tex Section "Foreign Function Declaration":

        extern "C" {
            fn f(x1: T1, ..., xn: Tn) -> R;
        }

    An extern declaration describes a function implemented in another
    language (typically C) that Brrr code can call. This establishes
    the contract between Brrr and the foreign code:

    1. SYMBOL NAME
       The function name must match the exported symbol in the foreign library.
       Use fn_link_name for symbol renaming (e.g., C++ mangled names).

    2. CALLING CONVENTION
       Must match what the foreign function expects. Mismatch causes UB.
       Default is CC_C for maximum compatibility.

    3. PARAMETER TYPES
       All parameters must be FFI-safe types with well-defined C ABI.
       Named parameters enable contract annotations (e.g., nonnull, out).

    4. VARIADIC FUNCTIONS
       Printf-family functions use variadic calling convention.
       Variadic functions have special type checking - only fixed params
       are validated; variadic args are checked via format string analysis.

    5. VISIBILITY
       Controls whether this declaration is exported from the generated library.
       FFIPublic functions appear in the symbol table with #[no_mangle].

    KARAMEL EXTRACTION:

    extern_fn declarations are extracted to C extern declarations:

        extern int32_t my_function(int32_t param1, uint8_t* param2);
    ...
    - Calling an extern function always requires unsafe context
    - The type system trusts the declaration; mismatches cause UB
    - Memory safety of foreign code is the programmer's responsibility
    ============================================================================ *)

(** Foreign function declaration with visibility control.

    Represents a function imported from or exported to foreign (non-Brrr) code.
    This is the F-star representation of the "extern" syntax from the spec.

    Fields:
    - fn_name:        Symbol name as it appears in object file
    - fn_convention:  Calling convention (CC_C is safest default)
    - fn_params:      Named parameters with FFI-safe types
    - fn_return:      Return type (must be FFI-safe or void)
    - fn_is_variadic: True for printf-style varargs functions
    - fn_link_name:   Optional override for linker symbol (for mangled names)
    - fn_visibility:  Export visibility (FFIPublic for shared library export) *)
noeq type extern_fn = {
...
(* ============================================================================
    ...
    The unsafe system provides controlled escape hatches from Brrr's
    safety guarantees. Unlike Rust where unsafe is purely syntactic,
    Brrr tracks unsafe effects in the type system (EUnsafe effect).

    UNSAFE OPERATION CATEGORIES:

    1. UnsafeFFI - Foreign Function Calls
       Why unsafe: Foreign code may violate any memory safety invariant.
       The Brrr type system cannot verify foreign code.

    2. UnsafePtr - Raw Pointer Operations
       Why unsafe: Dereferencing may access freed memory, cause UAF,
       or violate aliasing invariants.

    3. UnsafeTransmute - Type Transmutation
       Why unsafe: Reinterpreting bits as different type can create
       invalid values (e.g., invalid enum discriminants, null refs).

    4. UnsafeAsm - Inline Assembly
       Why unsafe: Assembly can do anything the processor allows.
       No type checking, no bounds checking, no aliasing analysis.

    5. UnsafeUnion - Union Field Access
       Why unsafe: Reading a union field that wasn't last written
       produces undefined behavior (type punning).

    UNSAFE CONTEXT PROPAGATION:

    The unsafe_context tracks which unsafe operations are permitted:
    - In normal code: no unsafe operations allowed
    - In unsafe block: all unsafe operations allowed
    - In unsafe function: all unsafe operations allowed (entire body is unsafe)

    EFFECT SYSTEM INTEGRATION:

    The EUnsafe effect (from BrrrEffects.fst) tracks that a function may
    perform unsafe operations. This effect must be discharged by an
    enclosing unsafe block:

        let risky : unit -> EUnsafe unit = ...

        let safe : unit -> unit =
          unsafe { risky () }  // EUnsafe discharged here

    SPECIFICATION REFERENCE:
    From effect row: "FFI: Foreign function call" (Section "Effect Kinds")
    ============================================================================ *)

(** Unsafe operation kinds.

    Each variant represents a category of operation that requires an
    unsafe context because the type system cannot verify its safety. *)
type unsafe_effect =
  | UnsafeFFI       : unsafe_effect   (* Foreign function call - cannot verify foreign code *)
  | UnsafePtr       : unsafe_effect   (* Raw pointer operations - may dereference invalid memory *)
  | UnsafeTransmute : unsafe_effect   (* Type transmutation - may create invalid values *)
  | UnsafeAsm       : unsafe_effect   (* Inline assembly - unrestricted machine access *)
  | UnsafeUnion     : unsafe_effect   (* Union field access - may read uninitialized bits *)
...
(* ============================================================================
    ...
    Printf-family functions (printf, sprintf, fprintf, etc.) are a major
    source of security vulnerabilities in C code:

    1. FORMAT STRING INJECTION (CWE-134)
       If user input becomes the format string: printf(user_input)
       Attacker can read stack memory (%x) or write arbitrary memory (%n).

    2. TYPE MISMATCH
       printf("%d", 3.14) - undefined behavior, reads garbage.
       printf("%s", 42) - reads memory at address 42, likely crash.

    3. ARGUMENT COUNT MISMATCH
       printf("%s %s", "hello") - reads uninitialized stack memory.
       printf("%s", "a", "b") - extra arg ignored, but code smell.

    4. BUFFER OVERFLOW
       sprintf(buf, "%s", very_long_string) - classic overflow.
       Even snprintf with %s can overflow if string is unterminated.

    THIS MODULE PROVIDES:

    - Parsing of printf format strings
    - Type checking of format specifiers against actual arguments
    - Warnings for dangerous patterns (%n, unbounded %s)
    - Support for length modifiers (l, ll, h, hh, z, t, j)

    SECURITY RECOMMENDATIONS:

    1. Never use user input as format string
    2. Always use snprintf, never sprintf
    3. Use %.Ns to limit string length: printf("%.10s", str)
    4. Disable %n entirely (this module warns on any use)
    5. Consider compile-time format checking (this module enables it)

    SUPPORTED FORMAT SPECIFIERS (C99/POSIX):
    %d, %i     - signed int (promoted from char/short in varargs)
    %u         - unsigned int
    %x, %X     - unsigned int (hexadecimal)
    %o         - unsigned int (octal)
    %f, %F     - double (promoted from float in varargs)
    %e, %E     - double (scientific notation)
    %g, %G     - double (auto-select f or e)
    %c         - int (converted to char for output)
    %s         - const char* (null-terminated string)
    %p         - void* (pointer, implementation-defined format)
    %n         - int* (DANGEROUS: writes count of chars printed)
    %%         - literal percent sign

    LENGTH MODIFIERS:
    hh         - char (signed or unsigned)
    h          - short (signed or unsigned)
    l          - long (or wchar_t for %c/%s)
    ll         - long long
    L          - long double (for %f/%e/%g)
    z          - size_t (for %d/%u/%x/%o)
    j          - intmax_t/uintmax_t
    t          - ptrdiff_t

    PLATFORM DEPENDENCY:

    Length modifiers l and L have platform-dependent sizes!
    - %ld: 4 bytes on LLP64 (Windows x64), 8 bytes on LP64 (Linux x64)
    - Use explicit %d for 32-bit, %lld for 64-bit to be portable.
    ============================================================================ *)
...
(* ============================================================================
    ...
    Representation attributes control memory layout of types. These are
    critical for FFI because the caller and callee must agree on layout.

    RUST/BRRR REPR ATTRIBUTES:

    #[repr(C)]          - C-compatible layout (required for FFI structs)
    #[repr(transparent)]- Single-field wrapper with same layout as inner type
    #[repr(packed)]     - No padding between fields (may cause unaligned access)
    #[repr(align(N))]   - Minimum alignment, useful for cache-line or SIMD

    WHY REPR(C) IS REQUIRED FOR FFI:

    Without repr(C), Rust/Brrr may:
    - Reorder fields for better packing
    - Use different padding rules
    - Apply optimization transformations

    With repr(C), layout follows C ABI rules:
    - Fields in declaration order
    - Standard padding for alignment
    - Tail padding to struct alignment

    REPR(TRANSPARENT) USE CASE:

    For newtype wrappers that should be FFI-safe:

        #[repr(transparent)]
        struct UserId(u64);  // Same ABI as u64

    This allows passing UserId to C functions expecting uint64_t.

    REPR(PACKED) WARNING:

    Packed structs can cause unaligned access which:
    - Crashes on strict-alignment architectures (older ARM, SPARC)
    - Is slower on x86 (multiple memory accesses)
    - Is non-atomic (concurrent access is unsafe)

    Only use packed for wire protocols where exact byte layout matters.
    ============================================================================ *)

(** Extended repr attribute for FFI (complements BrrrTypes.repr_attr).

    These attributes map to Rust #[repr(...)] and C __attribute__((...)).

    FReprC:           C-compatible layout - fields in declaration order
                      with standard C ABI padding rules.

    FReprTransparent: Same layout as the single field it contains.
                      Enables newtypes to be FFI-compatible.

    FReprPacked:      No padding between fields. CAUTION: May cause
                      unaligned access faults on some architectures.

    FReprAlign:       Minimum alignment. Struct is aligned to at least
                      this value, even if fields don't require it.

    FReprSimd:        SIMD-aligned (typically 16 or 32 bytes for SSE/AVX).
                      Use for types passed to SIMD intrinsics. *)
type ffi_repr_attr =
...
(* ============================================================================
    ...
    This section provides utilities for pointer arithmetic and casting
    analysis. These are critical for FFI because:

    1. POINTER ARITHMETIC SAFETY
       C allows arbitrary pointer arithmetic; Brrr must verify:
       - Result is within bounds of allocated object
       - Result is properly aligned for element type
       - No overflow in offset calculation

    2. POINTER CAST SAFETY
       Casting between pointer types has varying safety:
       - T* -> void* -> T*: Safe roundtrip
       - T* -> U*: May violate alignment if alignof(U) > alignof(T)
       - char* -> T*: Common for serialization, requires alignment check

    3. STRICT ALIASING
       C compilers assume pointers of different types don't alias.
       Casting and dereferencing through "wrong" type is UB.
       Only char* may alias any type (for serialization).

    POINTER CAST CLASSIFICATION:

    PtrCastSafe:        Same type or trivial (same size/align)
    PtrCastUpcast:      To larger alignment - always safe (strict subset)
    PtrCastDowncast:    To smaller alignment - may cause unaligned access
    PtrCastReinterpret: Different types - aliasing and alignment risks

    LOW-STAR REFERENCE:

    This approach mirrors LowStar.Buffer where pointer operations are
    verified for bounds and alignment at the F-star level, then extracted
    to raw C pointer operations.
    ============================================================================ *)

(** Pointer offset calculation.

    Computes byte offset for array element access:
    ptr_offset base index size = base + index * element_size

    This is the fundamental operation underlying array indexing. *)
...
(* ============================================================================
    ...
    An FFI contract is a formal specification of what a foreign function
    expects (preconditions) and guarantees (postconditions). This enables
    verified cross-language interoperability where the caller (Brrr) and
    callee (C/Rust/etc.) agree on a precise interface.

    VERIFFY APPROACH (Mansky et al., PLDI 2020):

    The VeriFFI methodology verifies that:
    1. Caller satisfies preconditions before the call
    2. Caller correctly interprets postconditions after the call
    3. Memory and ownership are correctly transferred

    This module implements a simplified version suitable for static analysis.

    KEY COMPONENTS:

    1. TYPE MAPPINGS (type_mapping)
       How values are transformed when crossing the boundary:
       - TMDirect:    Same representation, no conversion (e.g., i32 <-> int)
       - TMBoxed:     Heap wrapper (e.g., Python int <-> boxed integer)
       - TMConverted: Explicit conversion (e.g., String <-> char* with encoding)
       - TMOpaque:    Treat as opaque handle (e.g., FILE-ptr <-> void-ptr)

    2. REPRESENTATION REQUIREMENTS (rep_requirement)
       What properties a value must have:
       - RepNonNull:     Pointer is not null
       - RepInitialized: Memory is fully initialized
       - RepAligned:     Memory has proper alignment
       - RepSized:       Memory has minimum size

    3. PRECONDITIONS (ffi_precondition)
       Caller obligations before the call:
       - Per-parameter representation requirements
       - Per-parameter ownership/permission requirements
       - Thread safety requirements

    4. POSTCONDITIONS (ffi_postcondition)
       Callee guarantees after the call:
       - Return value representation
       - Memory effects (what was modified)
       - Ownership changes

    5. OWNERSHIP TRANSFER (ownership_transfer)
       How ownership moves during the call:
       - OTRetained:    Caller keeps ownership
       - OTTransferred: Ownership moves to callee (consumed)
       - OTBorrowed:    Temporary loan for call duration
       - OTShared:      Shared access granted

    VERIFICATION WORKFLOW:

    1. Parse/infer contract for extern function
    2. At call site, check preconditions against abstract state
    3. If preconditions satisfied, apply postcondition to update state
    4. If preconditions violated, report detailed error
    ...
    FUTURE WORK:
    - Integrate with separation logic assertions (from BrrrEffects.fst)
    - Support for caller-saved/callee-saved register conventions
    - Automatic contract inference from C header annotations
    ============================================================================ *)
...
(* IR value placeholder - actual values from BrrrValues.fst *)
...
(* ============================================================================
    ...
    and cross-language code analysis. This is critical for:

    - Libraries distributed as source that compile on multiple platforms
    - Network protocols where data is serialized/deserialized
    - File formats that must be readable across platforms
    - FFI where caller and callee may be compiled differently

    REAL-WORLD ABI PROBLEMS THIS PREVENTS:

    1. THE WINDOWS LONG PROBLEM
       On Windows x64 (LLP64): sizeof(long) == 4
       On Linux x64 (LP64):    sizeof(long) == 8
       Sharing structs with 'long' fields between Windows and Linux = corruption.

    2. THE STRUCT PADDING PROBLEM
       struct { char a; int b; char c; } has size 12 on most platforms
       but size 6 with #pragma pack(1). Mismatched packing = field offset errors.

    3. THE ENDIANNESS PROBLEM
       Big-endian stores 0x12345678 as [12, 34, 56, 78]
       Little-endian stores it as [78, 56, 34, 12]
       Sending binary data between architectures without conversion = garbage.

    4. THE ALIGNMENT PROBLEM
       ARM (pre-v7) requires aligned memory access.
       x86 allows unaligned access but with performance penalty.
       Packed structs sent to ARM code may cause SIGBUS.
    ...
    1. ENDIANNESS TRACKING
       ...
       - Generate byte swap requirements per field

    2. TARGET ABI CONFIGURATION (target_abi)
       - Architecture (x86, x64, ARM, ARM64, RISC-V, WASM)
       - Operating system (Linux, Windows, macOS, FreeBSD, iOS, Android)
    ...
    3. LAYOUT COMPUTATION
       ...
       - Field offset computation with proper padding
    ...
    4. ABI MISMATCH DETECTION
       ...
       - EndiannessMismatch: Byte order differs

    5. PLATFORM DATABASE
       - Predefined configurations for common platforms
       - Linux x64, Windows x64, macOS ARM64, etc.
       - Portable code verification against all common targets

    6. GUARD GENERATION
       - Generate C static_assert for sizeof/alignof/offsetof
       - Compile-time verification that ABI assumptions hold

    VERIFICATION STRATEGY:

    1. Compute layout for source ABI
    2. Compute layout for target ABI
    3. Compare field-by-field
    4. Report all mismatches with severity levels
    5. For portable code: check against all common ABIs

    CROSS-REFERENCES:
    - synthesis_combined.md Section "Gap 4: ABI Compatibility Checking"
    - Section 9.1.2: Boundary risk mitigation
    - Section 7.5: Representation predicates
    - EverParse: LowParse serialization patterns for portable formats
    ============================================================================ *)
...
(* ============================================================================
    ...
    The target_abi type fully specifies a platform's ABI conventions.
    This enables accurate cross-compilation and cross-platform analysis.

    DATA MODEL VARIANTS (see https://en.wikipedia.org/wiki/64-bit_computing):

    +----------+------+------+------+------+--------+------+
    | Model    | int  | long | ptr  | LL   | Where  | long |
    +----------+------+------+------+------+--------+------+
    | ILP32    | 32   | 32   | 32   | 64   | 32-bit | 4    |
    | LP64     | 32   | 64   | 64   | 64   | Unix64 | 8    |
    | LLP64    | 32   | 32   | 64   | 64   | Win64  | 4    |
    +----------+------+------+------+------+--------+------+

    CRITICAL: The key difference is 'long':
    - Linux/macOS x64: long is 8 bytes (LP64)
    - Windows x64: long is 4 bytes (LLP64)
    This affects structs with 'long' fields and size_t on some platforms.

    WHY TRACK ALL THESE SIZES?

    Even though we use fixed-width types (i32 not int), we still need:
    - abi_long_size for printf format specifiers (%ld)
    - abi_size_t_size for size-related operations
    - abi_max_align for SIMD types and cache-line alignment
    - abi_calling_conv for varargs and register allocation
    ============================================================================ *)
...
(** Complete target ABI specification.
    ...
    1. Compute struct layouts that match the target compiler
    2. Generate correct calling convention code
    3. Validate ABI compatibility between platforms
    4. Generate sizeof/alignof/offsetof assertions *)
noeq type target_abi = {
  abi_arch           : arch;              (* Target architecture (affects registers, SIMD) *)
  abi_os             : target_os;         (* Target operating system (affects syscalls, TLS) *)
  abi_endianness     : endianness;        (* Byte order for multi-byte values *)
  abi_pointer_size   : nat;               (* Pointer width in bytes (4 or 8) *)
  abi_i8_size        : nat;               (* i8/char size (always 1, included for completeness) *)
  ...
  abi_i128_size      : nat;               (* i128 size (16, or may be emulated as pair) *)
  abi_long_size      : nat;               (* C 'long' size - THE critical LP64 vs LLP64 difference! *)
  abi_size_t_size    : nat;               (* size_t size - usually equals pointer_size *)
  abi_f32_size       : nat;               (* f32/float size (always 4, IEEE 754) *)
  abi_f64_size       : nat;               (* f64/double size (always 8, IEEE 754) *)
  abi_max_align      : nat;               (* Maximum natural alignment - affects SIMD types *)
  abi_packing        : packing_mode;      (* Default struct packing mode *)
  abi_calling_conv   : calling_convention (* Default calling convention for this OS *)
}
