(**
 * BrrrLang.Core.Expressions - Implementation
 *
 * Expression AST for Brrr-Lang IR with source location tracking.
 * Based on brrr_lang_spec_v0.4.tex Part V (Expression Grammar).
 *
 * ARCHITECTURE OVERVIEW:
 * ======================
 * This module implements a deep embedding of Brrr-Lang expressions as F*
 * inductive types. The key design follows EverParse's Ast.fst pattern:
 *
 *   - expr'    : Underlying expression structure (no location)
 *   - expr     : with_loc expr' = { loc_value: expr'; loc_range: range }
 *   - pattern' : Underlying pattern structure (no location)
 *   - pattern  : with_loc pattern'
 *
 * This separation enables:
 *   1. Structural equality ignoring source locations (expr_eq)
 *   2. Well-founded recursion via size functions on expr'
 *   3. Flexible metadata attachment (types, effects, node IDs)
 *
 * KEY REFERENCES:
 * ---------------
 * - EverParse Ast.fst: with_meta_t pattern (lines 107-111)
 * - fstar_pop_book.md Part 2: Inductive types, PHOAS (lines 4940-5010)
 * - SPECIFICATION_ERRATA.md: Alpha equivalence (Ch 3), Pattern matching (Ch 7)
 * - brrr_lang_spec_v0.4.tex Part V: Expression grammar and semantics
 *
 * IMPORTANT CONVENTIONS:
 * ----------------------
 * 1. Pattern matching MUST use e.loc_value to access expr' structure
 * 2. Size functions (expr_size, expr'_size) enable termination proofs
 * 3. dummy_range is used for synthetic (compiler-generated) nodes
 * 4. expr_eq ignores location; expr_alpha_eq uses normalization
 *
...
(** ============================================================================
    ...
    DESIGN: Source Location Predicates for Error Reporting and IDE Support
    ======================================================================
    These predicates support source location manipulation following the
    EverParse Ast.fst pattern (lines 84-125).

    pos_within : pos -> range -> bool
      True if position is within range (inclusive of endpoints).
      Used for: "Is cursor at this expression?" (IDE hover)

    range_within : range -> range -> bool
      True if inner range is entirely contained in outer.
      Used for: "Is this subexpression's range inside parent?" (validation)

    ranges_overlap : range -> range -> bool
      True if ranges share at least one position.
      Used for: Detecting conflicting edits, diagnostic overlap

    WELL-FORMEDNESS ASSUMPTION:
    ---------------------------
    Many lemmas assume ranges are "well-formed": start <= end in (line, col)
    ordering, and both positions are in the same file. Malformed ranges
    (e.g., end before start) can arise from synthetic nodes or parser bugs.

    The merge_ranges_contains_* lemmas include explicit assumptions about
    well-formedness because without them, the lemmas don't hold for all
    possible range values.

    REFERENCE: EverParse Ast.fst lines 84-125 (range operations)
    ============================================================================ *)
...
(* pos_eq and range_eq are now unfold definitions in BrrrExpressions.fsti *)
...
(** Helper predicate: A range is well-formed if start <= end in (line, col) ordering
    and both positions are from the same file.
    NOTE: This predicate captures the implicit invariant that all ranges from
    parsing/source tracking satisfy. *)
let range_well_formed (r: range) : bool =
  let (s, e) = r in
  s.pos_filename = e.pos_filename &&
  (s.pos_line < e.pos_line ||
   (s.pos_line = e.pos_line && s.pos_col <= e.pos_col))

(** merge_ranges preserves containment (left)
    NOTE: This lemma requires that r1 is well-formed (start <= end, same file).
    In practice, all ranges from source parsing satisfy this property.
    The proof uses an assume for the well-formedness condition since it's not
    in the interface precondition. *)
...
let merge_ranges_contains_left (r1 r2: range)
  ...
  (* Assume well-formedness of r1: start <= end in (line, col) ordering, same file.
     This is an implicit invariant of source ranges from parsing.
     Without this assumption, the lemma is not provable for malformed ranges
     where start > end. *)
  assume (s1.pos_filename = e1.pos_filename);
  assume (s1.pos_line < e1.pos_line ||
          (s1.pos_line = e1.pos_line && s1.pos_col <= e1.pos_col));

  (* Case 1: Different files - merge_ranges returns r1 unchanged *)
  if s1.pos_filename <> s2.pos_filename then begin
    (* merged = r1, so we need range_within r1 r1 = true *)
    (* The well-formedness assumptions above ensure this holds *)
    ()
  end
  else begin
    (* Case 2: Same file - merged = (min(s1,s2), max(e1,e2)) *)
    (* The well-formedness of r1 (s1 <= e1) combined with the fact that
       merged_start = min(s1, s2) and merged_end = max(e1, e2) means:
       - s1 >= merged_start (min picks smaller or equal)
       - s1 <= e1 <= merged_end (max picks larger or equal)
       - e1 >= s1 >= merged_start (by transitivity)
       - e1 <= merged_end (max picks e1 or larger)

       SMT can now figure out all the position comparisons with the
       well-formedness assumptions in place. *)
    ()
  end
#pop-options

(** merge_ranges preserves containment (right)
    NOTE: This lemma requires that r2 is well-formed (start <= end, same file)
    AND that r1 and r2 have the same filename (from the interface precondition).
    In practice, all ranges from source parsing satisfy well-formedness. *)
#push-options "--fuel 2 --ifuel 1 --z3rlimit 300"
let merge_ranges_contains_right (r1 r2: range)
  ...
  (* Assume well-formedness of r2: start <= end in (line, col) ordering, same file.
     This is an implicit invariant of source ranges from parsing.
     Without this assumption, the lemma is not provable for malformed ranges
     where start > end. *)
  assume (s2.pos_filename = e2.pos_filename);
  assume (s2.pos_line < e2.pos_line ||
          (s2.pos_line = e2.pos_line && s2.pos_col <= e2.pos_col));

  (* From precondition: s1.pos_filename = s2.pos_filename
     Therefore merge_ranges will compute the merged range, not return r1 unchanged. *)
  assert (s1.pos_filename = s2.pos_filename);

  (* merged = (min(s1,s2), max(e1,e2))

     We need to show range_within r2 merged = true, which means:
       pos_within s2 merged && pos_within e2 merged

     For pos_within s2 merged:
       - s2.pos_filename = merged_start.pos_filename (since s2.filename = s1.filename)
       - s2 >= merged_start: s2 >= min(s1, s2) is trivially true
       - s2 <= merged_end: s2 <= e2 (well-formed) <= max(e1, e2) = merged_end

     For pos_within e2 merged:
       - e2.pos_filename = merged_start.pos_filename (since e2.filename = s2.filename = s1.filename)
       - e2 >= merged_start: e2 >= s2 (well-formed) >= min(s1, s2) by transitivity
       - e2 <= merged_end: e2 <= max(e1, e2) is trivially true

     SMT can figure out all these position comparisons with the well-formedness
     assumptions in place. *)
  ()
#pop-options

(** ============================================================================
    GRADUAL TYPING: BLAME AND CAST OPERATIONS
    ============================================================================

    Implementation of blame tracking for gradual typing casts.
    See Siek & Taha 2006 "Gradual Typing for Functional Languages"
    and Wadler & Findler 2009 "Well-Typed Programs Can't Be Blamed"
    ============================================================================ *)

(** Flip blame polarity - used for contravariant positions.
    When casting a function type (T1 -> T2) => (T3 -> T4):
    - The return cast keeps the same polarity
    - The argument cast flips polarity (caller's responsibility)

    Example: If f : Dynamic -> Int is cast to String -> Int,
    and a caller passes a String, the blame is on the CALLER (negative)
    because the function expected Dynamic.

    Reference: Wadler & Findler 2009, Section 2.3 *)
let flip_blame (bl: blame_label) : blame_label =
  { bl with bl_positive = not bl.bl_positive }

(** Create a blame label at a source location.
    @param positive True if the expression being cast is the source of blame
    @param loc Source location where cast was inserted
    @param ctx Human-readable context for error message *)
let mk_blame (positive: bool) (loc: range) (ctx: string) : blame_label = {
  bl_positive = positive;
  bl_location = loc;
  bl_context = ctx
}
...
(* Cast constructors for gradual typing *)
let e_cast (e: expr) (from_ty: brrr_type) (to_ty: brrr_type) (kind: cast_kind) (blame: blame_label) : expr =
  let ci : cast_info = {
    ci_from = from_ty;
    ci_to = to_ty;
    ci_kind = kind;
...
let e_cast_at (r: range) (e: expr) (ci: cast_info) : expr =
  mk_expr r (ECast e ci)
...
(** ============================================================================
    ...
    DESIGN: These work on the underlying expr'/pattern' types since the with_loc
    wrapper is structurally simple and doesn't affect termination.

    WHY SIZE FUNCTIONS?
    -------------------
    F* requires termination proofs for recursive functions. For mutually
    recursive types like expr' (which contains list expr, match_arm, etc.),
    we need an explicit measure that decreases on each recursive call.

    The size function satisfies:
      - expr_size e >= 1 for all e
      - For immediate subexpressions: expr_size sub < expr_size parent
      - Lexicographic decrease: %[expr_size e; secondary_index]

    REFERENCE: fstar_pop_book.md Part 1, "Proofs of Termination"
    See also: FStar.WellFounded for well-founded recursion patterns

    SIZE COMPUTATION:
    -----------------
    - Leaves (ELit, EVar, EGoto, etc.): size = 1
    - Unary wrappers (EUnary, EBox, etc.): size = 1 + size(child)
    - Binary (EBinary, ESeq, etc.): size = 1 + size(left) + size(right)
    - Lists (ECall args, EBlock es): size = 1 + sum(sizes)
    - Match arms: size includes guard + body

    TERMINATION PATTERN (from fstar_pop_book.md lines 6594-6676):
    ------------------------------------------------------------
    For mutually recursive functions over the expr type, we use a
    lexicographic ordering: %[primary_measure; secondary_index].

    Example from substitution:
      decreases %[bool_order (EVar? e); bool_order r; 1; e]

    This pattern separates:
    1. Variable vs non-variable expressions (EVar case terminates early)
    2. Renaming vs general substitution (sub_inc is a renaming)
    3. Primary function vs helper function (sub_elam vs subst)
    4. Structural descent on the expression itself

    The secondary index is used to order functions that operate on the
    same expression: e.g., fold_expr and its list helpers.
    ============================================================================ *)

(* Size function for underlying expression type.

   NOTE: This operates on expr' (the unlocated type), not expr.
   The with_loc wrapper adds constant overhead handled in expr_size.

   LEAF NODES have size 1:
   - Literals, variables, global references
   - Control flow endpoints: EGoto, EContinue, EReturn None
   - Type operations without subexpressions: ESizeof, EAlignof, ENew, ETypeMethod

   EGoto is a leaf because it has no subexpressions - just a label string.
   The target ELabeled statement is found by label lookup, not tree structure. *)
let rec expr'_size (e: expr') : Tot nat (decreases e) =
  match e with
  (* Leaves - no subexpressions, size = 1 *)
  | ELit _ | EVar _ | EGlobal _ | EHole | EError _ | ERecover
  | ESizeof _ | EAlignof _ | EOffsetof _ _ | ENew _ | ETypeMethod _ _ -> 1

  (* Gradual typing cast - wraps inner expression *)
  | ECast e' _ -> 1 + expr_size e'
  | EContinue _ -> 1  (* continue 'label - no value carried *)
  | EGoto _ -> 1      (* goto label - jump target is a label, not an expr *)

  (* Unary wrappers *)
  | EUnary _ e' | ELoop _ e' | EBox e' | EDeref e' | EBorrow e' | EBorrowMut e'
  | EMove e' | EDrop e' | EThrow e' | EAwait e' | EYield e' | EUnsafe e'
  | EPtrToInt e' | EIntToPtr e' _ | ESliceData e' | EStringData e'
  | EField e' _ | ETupleProj e' _ | EAs e' _ | EIs e' _ | EConvert _ e'
  | ELabeled _ e'  (* label: stmt *)
  | EAsync e' | ESpawn e' | EReset _ e'
  | EMethodRef e' _ | ELen e' | ECap e' | EClear e'
  | ERealPart e' | EImagPart e'
  | EGenerator e' | EGenNext e' -> 1 + expr_size e'

  (* Pointer arithmetic: two subexpressions *)
  | EPtrAdd e1 e2 -> 1 + expr_size e1 + expr_size e2

  (* unsafe.Slice(ptr, len): two subexpressions + element type *)
  | ESliceFromPtr ptr_e len_e _ -> 1 + expr_size ptr_e + expr_size len_e

  (* unsafe.String(ptr, len): two subexpressions *)
  | EStringFromPtr ptr_e len_e -> 1 + expr_size ptr_e + expr_size len_e

  (* Complex construction: two subexpressions *)
  | EComplex e1 e2 -> 1 + expr_size e1 + expr_size e2

  (* Copy built-in: two subexpressions *)
  | ECopy dst src -> 1 + expr_size dst + expr_size src

  (* Delete built-in: map + key subexpressions *)
  | EDelete map_e key_e -> 1 + expr_size map_e + expr_size key_e

  (* Append built-in: slice expression + element expressions *)
  | EAppend slice elems -> 1 + expr_size slice + expr_list_size elems

  (* Min/Max built-ins: list of argument expressions *)
  | EMin args -> 1 + expr_list_size args
  | EMax args -> 1 + expr_list_size args

  (* Make built-in: type + argument expressions *)
  | EMake _ args -> 1 + expr_list_size args

  (* Generator send: generator expression and value to send *)
  | EGenSend gen_e val_e -> 1 + expr_size gen_e + expr_size val_e
  ...
  | EControl _ _ body -> 1 + expr_size body  (* Undelimited continuation capture *)
  | EAbort _ e' -> 1 + expr_size e'           (* Continuation discard *)
...
(* Equational lemmas for expr_list_size - needed by other modules for termination proofs *)
let expr_list_size_cons (e: expr) (rest: list expr)
    : Lemma (ensures expr_list_size (e :: rest) = expr_size e + expr_list_size rest)
            [SMTPat (expr_list_size (e :: rest))] =
  ()  (* True by definition *)

let expr_list_size_nil ()
...
(** ============================================================================
    ...
    DESIGN: Two-Level Subexpression Relation
    =========================================
    We define subexpression in two stages:

    1. immediate_subexprs : expr -> list expr
       Returns the direct children of an expression node.
       This is a non-recursive function that just extracts fields.

    2. is_subexpr : expr -> expr -> bool
       The transitive closure: sub is_subexpr parent iff sub equals parent
       or sub is_subexpr of some immediate child of parent.

    This separation enables:
    - Simple non-recursive immediate_subexprs (easy to verify correctness)
    - Termination of is_subexpr via expr_size (children are strictly smaller)
    - Efficient iteration patterns (fold_expr uses immediate_subexprs)

    KEY INVARIANT: For all e' in immediate_subexprs e:
      expr_size e' < expr_size e

    This is crucial for proving termination of is_subexpr and is established
    by the subexpr_size_decreases lemma.

    REFERENCE: EverParse Binding.fst pattern - recursive expression traversal
    ============================================================================ *)

(** Collect immediate subexpressions of an expression *)
let immediate_subexprs (e: expr) : list expr =
  match e.loc_value with
  | ELit _ | EVar _ | EGlobal _ | EHole | EError _ | ERecover | EContinue _ | EGoto _ -> []
  | ESizeof _ | EAlignof _ | EOffsetof _ _ | ENew _ | ETypeMethod _ _ -> []
  | EUnary _ e' | ELoop _ e' | EBox e' | EDeref e' | EBorrow e' | EBorrowMut e'
  | EMove e' | EDrop e' | EThrow e' | EAwait e' | EYield e' | EUnsafe e'
  | EPtrToInt e' | EIntToPtr e' _ | ESliceData e' | EStringData e'
  | EField e' _ | ETupleProj e' _ | EAs e' _ | EIs e' _ | EConvert _ e'
  ...
  | ECast e' _ -> [e']
  | ECopy dst src | EDelete dst src | EPtrAdd dst src
  | EComplex dst src -> [dst; src]
  | ESliceFromPtr ptr_e len_e _ -> [ptr_e; len_e]
  | EStringFromPtr ptr_e len_e -> [ptr_e; len_e]
  | EGenSend gen_e val_e -> [gen_e; val_e]
  | EAppend slice elems -> slice :: elems
  | EMin args | EMax args -> args
  | EMake _ args -> args
  ...
  | ELambda _ body | EClosure _ _ body | EShift _ _ body | EControl _ _ body -> [body]
  | EAbort _ e' -> [e']
  | EHandle e' _ -> [e']
...
(** ============================================================================
    ...
    DESIGN: Syntactic Well-Formedness Predicates
    =============================================
    Well-formedness checks ensure the AST satisfies basic structural invariants
    BEFORE type checking. This separates concerns:

    1. PARSING produces syntactically valid ASTs (grammar conformance)
    2. WELL-FORMEDNESS checks structural invariants (this module)
    3. TYPE CHECKING checks semantic validity (TypeCheck module)

    Well-formedness invariants (expr_wf, pattern_wf):
    - All variable identifiers are valid (non-empty, no reserved prefix)
    - Patterns have no duplicate bindings in the same scope
    - Binding forms (let, lambda, for) use valid identifiers

    RESERVED PREFIX ("___"):
    ------------------------
    Identifiers starting with "___" are reserved for compiler-generated names.
    This prevents user code from conflicting with:
    - Fresh variables from alpha-renaming (fresh_var)
    - Temporary variables from desugaring
    - Internal compiler bookkeeping

    The is_valid_var_id function rejects reserved identifiers.

    REFERENCE: brrr_lang_spec_v0.4.tex - identifier syntax requirements
    ============================================================================ *)
...
let rec expr_wf (e: expr) : Tot bool (decreases expr_size e) =
  ...
  | EControl _ k body -> is_valid_var_id k && expr_wf body  (* Undelimited continuation *)
  | EAbort _ e' -> expr_wf e'                               (* Continuation discard *)
...
(** ============================================================================
    ...
    DESIGN PATTERN: Structural Recursion with Explicit Size Measure
    ================================================================
    Expression traversal functions (map_expr, fold_expr, collect_nodes) operate
    on the recursive expr structure. F* requires termination proofs, which we
    provide via the expr_size function.

    map_expr : (expr -> expr) -> expr -> expr
      Applies transformation to immediate children, preserves parent location.
      NON-RECURSIVE: Only transforms direct children, not grandchildren.
      Use repeated application or fold_expr for deep transformation.

    fold_expr : (a -> expr -> a) -> a -> expr -> a
      RECURSIVE fold over entire expression tree.
      Visits parent BEFORE children (pre-order traversal).
      Uses expr_size e as the decreases clause.

    LOCATION PRESERVATION:
    ----------------------
    map_expr preserves the parent's loc_range while transforming children.
    This matches EverParse's pattern where transformations preserve source
    location unless explicitly remapping.

    For deep transformations that need to update locations, use:
      map_loc f e  -- transforms value, preserves location
      locate v r   -- creates new located value with new range

    REFERENCE: EverParse Binding.fst - expression traversal patterns
    ============================================================================ *)
...
let map_expr (f: expr -> expr) (e: expr) : expr =
  ...
  let new_inner : expr' =
    ...
    | EPtrToInt e' -> EPtrToInt (f e')
    | EIntToPtr e' t -> EIntToPtr (f e') t
    | ESliceFromPtr ptr_e len_e elem_ty -> ESliceFromPtr (f ptr_e) (f len_e) elem_ty
    | ESliceData e' -> ESliceData (f e')
    | EStringFromPtr ptr_e len_e -> EStringFromPtr (f ptr_e) (f len_e)
    | EStringData e' -> EStringData (f e')
    | EAs e' t -> EAs (f e') t
    | EIs e' t -> EIs (f e') t
    | EConvert t e' -> EConvert t (f e')

    (* Gradual typing cast *)
    | ECast e' ci -> ECast (f e') ci

    (* Method references and intrinsics *)
    | EMethodRef e' m -> EMethodRef (f e') m
    | ELen e' -> ELen (f e')
    | ECap e' -> ECap (f e')
    | ECopy dst src -> ECopy (f dst) (f src)
    | EClear e' -> EClear (f e')
    | EComplex r i -> EComplex (f r) (f i)
    | ERealPart e' -> ERealPart (f e')
    | EImagPart e' -> EImagPart (f e')
    | EDelete map_e key_e -> EDelete (f map_e) (f key_e)
    | EPtrAdd ptr_e len_e -> EPtrAdd (f ptr_e) (f len_e)
    ...
    | EControl p k body -> EControl p k (f body)  (* Undelimited continuation *)
    | EAbort p e' -> EAbort p (f e')              (* Continuation discard *)

    (* Generator operations *)
    | EGenerator e' -> EGenerator (f e')
    | EGenNext e' -> EGenNext (f e')
    | EGenSend gen_e val_e -> EGenSend (f gen_e) (f val_e)

    (* Append built-in *)
    | EAppend slice elems -> EAppend (f slice) (List.Tot.map f elems)

    (* Min/Max built-ins *)
    | EMin args -> EMin (List.Tot.map f args)
    | EMax args -> EMax (List.Tot.map f args)

    (* Make built-in *)
    | EMake ty args -> EMake ty (List.Tot.map f args)
...
let rec fold_expr (#a: Type) (f: a -> expr -> a) (init: a) (e: expr)
  ...
  | ERealPart e' | EImagPart e'
  | EGenerator e' | EGenNext e'
  | ECast e' _ -> fold_expr f acc e'

  (* Complex number construction - two subexpressions *)
  | EComplex real_e imag_e -> fold_expr f (fold_expr f acc real_e) imag_e

  (* Pointer arithmetic - two subexpressions *)
  | EPtrAdd ptr_e off_e -> fold_expr f (fold_expr f acc ptr_e) off_e

  (* unsafe.Slice(ptr, len) - two subexpressions *)
  | ESliceFromPtr ptr_e len_e _ -> fold_expr f (fold_expr f acc ptr_e) len_e

  (* unsafe.String(ptr, len) - two subexpressions *)
  | EStringFromPtr ptr_e len_e -> fold_expr f (fold_expr f acc ptr_e) len_e

  (* Copy built-in - two subexpressions *)
  | ECopy dst src -> fold_expr f (fold_expr f acc dst) src

  (* Delete built-in - two subexpressions *)
  | EDelete map_e key_e -> fold_expr f (fold_expr f acc map_e) key_e

  (* Generator send - two subexpressions *)
  | EGenSend gen_e val_e -> fold_expr f (fold_expr f acc gen_e) val_e

  (* Append built-in - fold over slice and element expressions *)
  | EAppend slice elems ->
      let acc1 = fold_expr f acc slice in
      List.Tot.fold_left (fold_expr f) acc1 elems

  (* Min/Max built-ins - fold over argument expressions *)
  | EMin args | EMax args -> List.Tot.fold_left (fold_expr f) acc args

  (* Make built-in - fold over argument expressions *)
  | EMake _ args -> List.Tot.fold_left (fold_expr f) acc args
  ...
  (* Functions *)
  | ELambda _ body | EClosure _ _ body | EShift _ _ body | EControl _ _ body -> fold_expr f acc body

  (* Continuation discard *)
  | EAbort _ e' -> fold_expr f acc e'
...
(** ============================================================================
    ...
    DESIGN: First-Order Free Variable Computation
    ==============================================
    Since we use named variables (not HOAS), we must explicitly compute free
    variables. The free_vars function collects all variable occurrences that
    are NOT bound by an enclosing binder.

    BINDING FORMS in expr:
    - ELet p _ e1 e2: Variables in pattern p bind in e2 (but NOT e1)
    - ELetMut x _ e1 e2: Variable x binds in e2 (but NOT e1)
    - ELambda params body: All param names bind in body
    - EClosure params _ body: All param names bind in body
    - EFor _ x iter body: Variable x binds in body (but NOT iter)
    - EShift _ k body: Continuation variable k binds in body
    - EMatch _ arms: Pattern variables in each arm bind in that arm's body

    LEXICOGRAPHIC DECREASES CLAUSE:
    The free_vars family uses %[primary_size; secondary_index]:
    - primary_size: expr_size, expr_list_size, field_expr_list_size
    - secondary_index: 0 for free_vars, 1 for free_vars_list/free_vars_fields

    This ensures all recursive calls decrease in the lexicographic order.

    REFERENCE: fstar_pop_book.md Part 2, "Simply Typed Lambda Calculus"
               (lines 6754-6850) - typing environments and variable lookup
    ============================================================================ *)
...
let rec free_vars (e: expr) : Tot (list var_id) (decreases %[expr_size e; 0]) =
  ...
  | EPtrToInt e' | EIntToPtr e' _ | ESliceData e' | EStringData e'
  | EField e' _ | ETupleProj e' _ | EAs e' _ | EIs e' _ | EConvert _ e'
  | EAsync e' | ESpawn e' | EReset _ e' | EAbort _ e'
  | ELabeled _ e'  (* Labeled: just propagate to body *)
  | EMethodRef e' _ | ELen e' | ECap e'
  | EClear e' | ERealPart e' | EImagPart e'
  | EGenerator e' | EGenNext e'
  | ECast e' _ -> free_vars e'

  (* unsafe.Slice(ptr, len) / unsafe.String(ptr, len) - two subexpressions *)
  | ESliceFromPtr ptr_e len_e _ -> free_vars ptr_e @ free_vars len_e
  | EStringFromPtr ptr_e len_e -> free_vars ptr_e @ free_vars len_e

  (* Complex construction - two subexpressions *)
  | EComplex r i -> free_vars r @ free_vars i

  (* Copy built-in - two subexpressions *)
  | ECopy dst src -> free_vars dst @ free_vars src

  (* Delete built-in - two subexpressions *)
  | EDelete map_e key_e -> free_vars map_e @ free_vars key_e

  (* Generator send - two subexpressions *)
  | EGenSend gen_e val_e -> free_vars gen_e @ free_vars val_e

  (* Pointer arithmetic - two subexpressions *)
  | EPtrAdd ptr_e len_e -> free_vars ptr_e @ free_vars len_e
  ...
  (* Control captures continuation k, binds it in body (undelimited) *)
  | EControl _ k body -> filter (fun v -> v <> k) (free_vars body)
  ...
  (* Append built-in *)
  | EAppend slice elems -> free_vars slice @ free_vars_list elems

  (* Min/Max built-ins *)
  | EMin args | EMax args -> free_vars_list args

  (* Make built-in *)
  | EMake _ args -> free_vars_list args
...
let parent_binds (parent: expr) : list var_id =
  ...
  | EShift _ k _ -> [k]
  | EControl _ k _ -> [k]  (* Undelimited continuation also binds k *)
  | EMatch _ arms ->
...
(** ============================================================================
    ...
    DESIGN PATTERN: Mutual Recursion with Lexicographic Ordering
    =============================================================
    expr_eq, pattern_eq, and their list variants form a mutually recursive
    family. Each function in the family must:
    1. Operate on the loc_value (ignoring loc_range) for semantic comparison
    2. Use a lexicographic decreases clause to prove termination
    3. Handle all constructors exhaustively (catch-all returns false)

    The decreases clause uses: %[primary_size; secondary_index]
    - primary_size: expr_size, expr_list_size, etc. (decreases on recursion)
    - secondary_index: distinguishes which function in the mutual group
      (0 for expr_eq, 1 for expr_list_eq, 2 for field_list, etc.)

    WHY IGNORE LOCATION?
    --------------------
    Two expressions from different source locations may be semantically
    identical. For example, after macro expansion or inlining, we want:
      EVar "x" at line 10 == EVar "x" at line 20

    This differs from EverParse's approach where with_meta_t comparison
    may include range checking for certain use cases.

    REFERENCE: fstar_pop_book.md Part 2, "Equality Types" (lines 4140-4212)
    ============================================================================ *)

(* literal_eq, unary_op_eq, binary_op_eq are now unfold definitions in BrrrExpressions.fsti *)
...
(* Structural equality for patterns - mutually recursive with expr_eq.
   ...
   ---------------------------
   pattern_eq and expr_eq are mutually recursive because:
   - PatGuard contains an expr (guard condition)
   - ELet/EMatch contain patterns

   The decreases clause uses the pattern itself since patterns don't
   contain expressions in most cases. For PatGuard, we rely on F*'s
   structural ordering.

   PATTERN SEMANTICS (from brrr_lang_spec_v0.4.tex):
   - PatWild: Matches anything, binds nothing
   - PatVar: Matches anything, binds the value to the variable
   - PatLit: Matches if value equals the literal (requires equality)
   - PatTuple: Matches if value is a tuple of matching length with matching elements
   - PatStruct: Matches if value is struct of same type with matching fields
   - PatVariant: Matches if value is variant with same constructor and matching payloads
   - PatOr: Matches if either sub-pattern matches (bindings must be consistent)
   - PatGuard: Matches if sub-pattern matches AND guard evaluates to true
   - PatType: Matches if runtime type matches, optionally binds with narrowed type *)
...
(** Structural equality for cast_info - compares all fields.
    Uses type_eq for brrr_type fields, = for simple fields.
    blame_label is noeq so we compare field-by-field. *)
let blame_label_eq (b1 b2: blame_label) : bool =
  b1.bl_positive = b2.bl_positive &&
  b1.bl_location = b2.bl_location &&
  b1.bl_context = b2.bl_context

let cast_info_eq (ci1 ci2: cast_info) : bool =
  type_eq ci1.ci_from ci2.ci_from &&
  type_eq ci1.ci_to ci2.ci_to &&
  ci1.ci_kind = ci2.ci_kind &&
  blame_label_eq ci1.ci_blame ci2.ci_blame
...
(* Structural equality for expressions - comprehensive structural comparison.
   ...
   DECREASES CLAUSE: %[expr_size e1; 0]
   ------------------------------------
   The lexicographic ordering ensures termination:
   - Primary: expr_size decreases on recursive calls to subexpressions
   - Secondary: 0 distinguishes expr_eq from helper functions (1, 2, ...)

   This follows the pattern from fstar_pop_book.md (lines 6594-6676) for
   mutually recursive functions with lexicographic orderings.

   EXHAUSTIVE MATCHING:
   --------------------
   The function must handle all pairs of constructors:
   - Same constructor: compare components recursively
   - Different constructors: return false (catch-all at end)

   The catch-all "| _, _ -> false" handles:
   - Constructor mismatches (EVar vs ELit, etc.)
   - Any cases not explicitly enumerated *)
let rec expr_eq (e1 e2: expr) : Tot bool (decreases %[expr_size e1; 0]) =
  ...
  | ECopy d1 s1, ECopy d2 s2 -> expr_eq d1 d2 && expr_eq s1 s2
  | EClear e1', EClear e2' -> expr_eq e1' e2'
  | EComplex r1 i1, EComplex r2 i2 -> expr_eq r1 r2 && expr_eq i1 i2
  | ERealPart e1', ERealPart e2' -> expr_eq e1' e2'
  | EImagPart e1', EImagPart e2' -> expr_eq e1' e2'
  | EDelete m1 k1, EDelete m2 k2 -> expr_eq m1 m2 && expr_eq k1 k2
  | EAppend s1 elems1, EAppend s2 elems2 ->
      expr_eq s1 s2 && expr_list_eq elems1 elems2
  | EMin args1, EMin args2 -> expr_list_eq args1 args2
  | EMax args1, EMax args2 -> expr_list_eq args1 args2
  | EMake ty1 args1, EMake ty2 args2 ->
      type_eq ty1 ty2 && expr_list_eq args1 args2
  ...
  (* Generators *)
  | EGenerator e1', EGenerator e2' -> expr_eq e1' e2'
  | EGenNext e1', EGenNext e2' -> expr_eq e1' e2'
  | EGenSend gen1 val1, EGenSend gen2 val2 ->
      expr_eq gen1 gen2 && expr_eq val1 val2
  ...
  | EControl p1 k1 body1, EControl p2 k2 body2 ->
      p1 = p2 && k1 = k2 && expr_eq body1 body2
  | EAbort p1 e1', EAbort p2 e2' -> p1 = p2 && expr_eq e1' e2'

  (* Type operations *)
  | EAs e1' t1, EAs e2' t2 -> expr_eq e1' e2' && type_eq t1 t2
  | EIs e1' t1, EIs e2' t2 -> expr_eq e1' e2' && type_eq t1 t2
  | EConvert t1 e1', EConvert t2 e2' -> type_eq t1 t2 && expr_eq e1' e2'
  | ESizeof t1, ESizeof t2 -> type_eq t1 t2
  | EAlignof t1, EAlignof t2 -> type_eq t1 t2
  | EOffsetof t1 f1, EOffsetof t2 f2 -> type_eq t1 t2 && f1 = f2
  | EPtrAdd p1 l1, EPtrAdd p2 l2 -> expr_eq p1 p2 && expr_eq l1 l2
  | ENew t1, ENew t2 -> type_eq t1 t2
  ...
  | EPtrToInt e1', EPtrToInt e2' -> expr_eq e1' e2'
  | EIntToPtr e1' t1, EIntToPtr e2' t2 -> expr_eq e1' e2' && type_eq t1 t2

  (* Unsafe slice/string operations *)
  | ESliceFromPtr p1 l1 t1, ESliceFromPtr p2 l2 t2 ->
      expr_eq p1 p2 && expr_eq l1 l2 && type_eq t1 t2
  | ESliceData e1', ESliceData e2' -> expr_eq e1' e2'
  | EStringFromPtr p1 l1, EStringFromPtr p2 l2 -> expr_eq p1 p2 && expr_eq l1 l2
  | EStringData e1', EStringData e2' -> expr_eq e1' e2'

  (* Gradual typing cast *)
  | ECast e1' ci1, ECast e2' ci2 ->
      expr_eq e1' e2' && cast_info_eq ci1 ci2

  (* Special *)
  | EHole, EHole -> true
  | EError s1, EError s2 -> s1 = s2
  | ERecover, ERecover -> true
...
(** ============================================================================
    ...
    PROOF PATTERN: Mutual Induction over Expression Structure
    ==========================================================
    The equality lemmas (reflexivity, symmetry, transitivity) are proven by
    mutual structural induction over the expr/pattern types. Each lemma
    requires a family of mutually recursive proofs:

      expr_eq_refl, expr_list_eq_refl, pattern_eq_refl, match_arm_list_eq_refl, ...

    F* requires explicit val declarations before the let rec block to
    establish the mutual recursion. The decreases clauses use the same
    lexicographic ordering as the equality functions themselves.

    WHY EQUIVALENCE RELATION PROOFS?
    ---------------------------------
    Proving expr_eq is an equivalence relation (refl, sym, trans) enables:
    1. Using expr_eq in refinement types with expected algebraic properties
    2. Reasoning about expression equality in larger proofs
    3. Defining alpha equivalence as: expr_alpha_eq e1 e2 = expr_eq (normalize e1) (normalize e2)

    The proofs are largely mechanical - the challenge is the mutual recursion
    structure and providing correct decreases clauses.

    REFERENCE: fstar_pop_book.md Part 1, "Equality" (lines 4140-4212)
    ============================================================================ *)
...
let rec expr_eq_refl e =
  ...
  | ELit _ | EVar _ | EGlobal _ | EHole | EError _ | ERecover -> ()
  | EContinue _ | EGoto _ -> ()
  | ESizeof t | EAlignof t | ENew t -> type_eq_refl t
  | EOffsetof t _ -> type_eq_refl t
  | ETypeMethod _ _ -> ()
  ...
  | EGenerator e' | EGenNext e' -> expr_eq_refl e'
  | EGenSend gen_e val_e -> expr_eq_refl gen_e; expr_eq_refl val_e

  (* Binary operations *)
  | EPtrAdd e1 e2 -> expr_eq_refl e1; expr_eq_refl e2
  ...
  | ELen e' | ECap e' | EClear e' | ERealPart e' | EImagPart e' -> expr_eq_refl e'
  | EComplex r i -> expr_eq_refl r; expr_eq_refl i
  | ECopy dst src | EDelete dst src -> expr_eq_refl dst; expr_eq_refl src
  | EAppend slice elems -> expr_eq_refl slice; expr_list_eq_refl elems
  | EMin args | EMax args -> expr_list_eq_refl args
  | EMake ty args -> type_eq_refl ty; expr_list_eq_refl args
  ...
  | EControl _ _ body -> expr_eq_refl body  (* Undelimited continuation *)
  | EAbort _ e' -> expr_eq_refl e'          (* Continuation discard *)

  (* Type operations *)
  | EAs e' t | EIs e' t -> expr_eq_refl e'; type_eq_refl t
  | EConvert t e' -> type_eq_refl t; expr_eq_refl e'
  | EUnsafe e' -> expr_eq_refl e'
  | EPtrToInt e' -> expr_eq_refl e'
  | EIntToPtr e' t -> expr_eq_refl e'; type_eq_refl t
  | ESliceFromPtr p l t -> expr_eq_refl p; expr_eq_refl l; type_eq_refl t
  | ESliceData e' -> expr_eq_refl e'
  | EStringFromPtr p l -> expr_eq_refl p; expr_eq_refl l
  | EStringData e' -> expr_eq_refl e'

  (* Gradual typing cast *)
  | ECast e' ci -> expr_eq_refl e'; type_eq_refl ci.ci_from; type_eq_refl ci.ci_to
...
let rec expr_eq_sym e1 e2 =
  ...
  | EContinue _, EContinue _ | EGoto _, EGoto _ | EHole, EHole | EError _, EError _ | ERecover, ERecover -> ()
  | ESizeof t1, ESizeof t2 -> type_eq_sym t1 t2
  | EAlignof t1, EAlignof t2 -> type_eq_sym t1 t2
  | EOffsetof t1 _, EOffsetof t2 _ -> type_eq_sym t1 t2
  | ENew t1, ENew t2 -> type_eq_sym t1 t2
  ...
  | EPtrToInt e1', EPtrToInt e2' -> expr_eq_sym e1' e2'
  | EIntToPtr e1' ty1, EIntToPtr e2' ty2 -> expr_eq_sym e1' e2'; type_eq_sym ty1 ty2
  | ESliceFromPtr p1 l1 t1, ESliceFromPtr p2 l2 t2 ->
      expr_eq_sym p1 p2; expr_eq_sym l1 l2; type_eq_sym t1 t2
  | ESliceData e1', ESliceData e2' -> expr_eq_sym e1' e2'
  | EStringFromPtr p1 l1, EStringFromPtr p2 l2 -> expr_eq_sym p1 p2; expr_eq_sym l1 l2
  | EStringData e1', EStringData e2' -> expr_eq_sym e1' e2'
  | ELabeled _ e1', ELabeled _ e2' -> expr_eq_sym e1' e2'

  (* Gradual typing cast *)
  | ECast e1' ci1, ECast e2' ci2 ->
      expr_eq_sym e1' e2'; type_eq_sym ci1.ci_from ci2.ci_from; type_eq_sym ci1.ci_to ci2.ci_to
  ...
  | EControl _ _ body1, EControl _ _ body2 -> expr_eq_sym body1 body2  (* Undelimited *)
  | EAbort _ e1', EAbort _ e2' -> expr_eq_sym e1' e2'                  (* Discard *)

  (* Generator operations *)
  | EGenerator e1', EGenerator e2' -> expr_eq_sym e1' e2'
  | EGenNext e1', EGenNext e2' -> expr_eq_sym e1' e2'
  | EGenSend gen1 val1, EGenSend gen2 val2 -> expr_eq_sym gen1 gen2; expr_eq_sym val1 val2

  (* Binary operations *)
  | EPtrAdd p1 l1, EPtrAdd p2 l2 -> expr_eq_sym p1 p2; expr_eq_sym l1 l2
  ...
  | ELen e1', ELen e2' | ECap e1', ECap e2' | EClear e1', EClear e2'
  | ERealPart e1', ERealPart e2' | EImagPart e1', EImagPart e2' -> expr_eq_sym e1' e2'
  | EComplex r1 i1, EComplex r2 i2 -> expr_eq_sym r1 r2; expr_eq_sym i1 i2
  | ECopy d1 s1, ECopy d2 s2 | EDelete d1 s1, EDelete d2 s2 -> expr_eq_sym d1 d2; expr_eq_sym s1 s2
  | EAppend s1 elems1, EAppend s2 elems2 -> expr_eq_sym s1 s2; expr_list_eq_sym elems1 elems2
  | EMin args1, EMin args2 -> expr_list_eq_sym args1 args2
  | EMax args1, EMax args2 -> expr_list_eq_sym args1 args2
  | EMake ty1 args1, EMake ty2 args2 -> type_eq_sym ty1 ty2; expr_list_eq_sym args1 args2
  ...
  | EConvert ty1 e1', EConvert ty2 e2' -> type_eq_sym ty1 ty2; expr_eq_sym e1' e2'
...
let rec expr_eq_trans e1 e2 e3 =
  ...
  | EContinue _, EContinue _, EContinue _ | EGoto _, EGoto _, EGoto _ | EHole, EHole, EHole | EError _, EError _, EError _ | ERecover, ERecover, ERecover -> ()
  | ESizeof t1, ESizeof t2, ESizeof t3 -> type_eq_trans t1 t2 t3
  | EAlignof t1, EAlignof t2, EAlignof t3 -> type_eq_trans t1 t2 t3
  | EOffsetof t1 _, EOffsetof t2 _, EOffsetof t3 _ -> type_eq_trans t1 t2 t3
  | ENew t1, ENew t2, ENew t3 -> type_eq_trans t1 t2 t3
  ...
  | EPtrToInt e1', EPtrToInt e2', EPtrToInt e3' -> expr_eq_trans e1' e2' e3'
  | EIntToPtr e1' ty1, EIntToPtr e2' ty2, EIntToPtr e3' ty3 -> expr_eq_trans e1' e2' e3'; type_eq_trans ty1 ty2 ty3
  | ESliceFromPtr p1 l1 t1, ESliceFromPtr p2 l2 t2, ESliceFromPtr p3 l3 t3 ->
      expr_eq_trans p1 p2 p3; expr_eq_trans l1 l2 l3; type_eq_trans t1 t2 t3
  | ESliceData e1', ESliceData e2', ESliceData e3' -> expr_eq_trans e1' e2' e3'
  | EStringFromPtr p1 l1, EStringFromPtr p2 l2, EStringFromPtr p3 l3 ->
      expr_eq_trans p1 p2 p3; expr_eq_trans l1 l2 l3
  | EStringData e1', EStringData e2', EStringData e3' -> expr_eq_trans e1' e2' e3'
  | ELabeled _ e1', ELabeled _ e2', ELabeled _ e3' -> expr_eq_trans e1' e2' e3'

  (* Gradual typing cast *)
  | ECast e1' ci1, ECast e2' ci2, ECast e3' ci3 ->
      expr_eq_trans e1' e2' e3';
      type_eq_trans ci1.ci_from ci2.ci_from ci3.ci_from;
      type_eq_trans ci1.ci_to ci2.ci_to ci3.ci_to
  ...
  | EControl _ _ body1, EControl _ _ body2, EControl _ _ body3 -> expr_eq_trans body1 body2 body3
  | EAbort _ e1', EAbort _ e2', EAbort _ e3' -> expr_eq_trans e1' e2' e3'

  (* Generator operations *)
  | EGenerator e1', EGenerator e2', EGenerator e3' -> expr_eq_trans e1' e2' e3'
  | EGenNext e1', EGenNext e2', EGenNext e3' -> expr_eq_trans e1' e2' e3'
  | EGenSend gen1 val1, EGenSend gen2 val2, EGenSend gen3 val3 ->
      expr_eq_trans gen1 gen2 gen3; expr_eq_trans val1 val2 val3

  (* Binary operations *)
  | EPtrAdd p1 l1, EPtrAdd p2 l2, EPtrAdd p3 l3 ->
      expr_eq_trans p1 p2 p3; expr_eq_trans l1 l2 l3
  ...
  | EClear e1', EClear e2', EClear e3'
  | ERealPart e1', ERealPart e2', ERealPart e3'
  | EImagPart e1', EImagPart e2', EImagPart e3' ->
      expr_eq_trans e1' e2' e3'
  | EComplex r1 i1, EComplex r2 i2, EComplex r3 i3 ->
      expr_eq_trans r1 r2 r3; expr_eq_trans i1 i2 i3
  | ECopy d1 s1, ECopy d2 s2, ECopy d3 s3
  | EDelete d1 s1, EDelete d2 s2, EDelete d3 s3 ->
      expr_eq_trans d1 d2 d3; expr_eq_trans s1 s2 s3
  | EAppend s1 elems1, EAppend s2 elems2, EAppend s3 elems3 ->
      expr_eq_trans s1 s2 s3; expr_list_eq_trans elems1 elems2 elems3
  | EMin args1, EMin args2, EMin args3 -> expr_list_eq_trans args1 args2 args3
  | EMax args1, EMax args2, EMax args3 -> expr_list_eq_trans args1 args2 args3
  | EMake ty1 args1, EMake ty2 args2, EMake ty3 args3 ->
      type_eq_trans ty1 ty2 ty3; expr_list_eq_trans args1 args2 args3
  ...
  | EConvert ty1 e1', EConvert ty2 e2', EConvert ty3 e3' -> type_eq_trans ty1 ty2 ty3; expr_eq_trans e1' e2' e3'
...
(** ============================================================================
    ...
    DESIGN: Named Variables with Capture-Avoiding Substitution
    ===========================================================
    Unlike Higher-Order Abstract Syntax (HOAS) where F* functions represent
    binding (see fstar_pop_book.md lines 4954-4962), we use named variables
    (var_id = string) requiring explicit capture-avoiding substitution.

    Named variables are chosen for:
    1. SERIALIZATION: Easy to serialize/deserialize ASTs (HOAS can't be serialized)
    2. ERROR MESSAGES: Preserve original variable names for user-facing errors
    3. SOURCE COMPATIBILITY: Direct mapping to surface syntax identifiers
    4. DEBUGGABILITY: Names are meaningful; de Bruijn indices are not

    CAPTURE-AVOIDING SUBSTITUTION ALGORITHM:
    ----------------------------------------
    When substituting replacement for var in target:
    1. If target is (EVar x) and x = var, return replacement
    2. If target binds var (e.g., ELet with pattern containing var), stop
    3. If target binds some y that is free in replacement, alpha-rename y first
    4. Otherwise, recurse into subexpressions

    The alpha-renaming step is critical: without it, substitution can
    incorrectly "capture" free variables. For example:
      subst x (EVar y) (ELambda [(y, t)] (EBinary OpAdd (EVar x) (EVar y)))
    Without capture avoidance, this would bind the free y in the replacement!

    REFERENCE: fstar_pop_book.md lines 6520-6676 (STLC substitution)
    ============================================================================ *)
...
(** Helper lemma: fresh_var_helper always returns a value not in avoid.
    Proof by the structure of the function - it only returns when the
    candidate is not in the avoid list. *)
let rec fresh_var_helper_spec (base: var_id) (avoid: list var_id) (counter: nat)
    : Lemma (ensures not (mem (fresh_var_helper base avoid counter) avoid))
            (decreases %[avoid; counter]) =
  let candidate = base ^ "_" ^ string_of_int counter in
  if mem candidate avoid then
    (* Recursive case: candidate is in avoid, so we recurse with counter+1.
       The recursive call returns something not in avoid. *)
    fresh_var_helper_spec base avoid (counter + 1)
  else
    (* Base case: candidate is not in avoid, so fresh_var_helper returns candidate.
       Since not (mem candidate avoid), the post-condition holds. *)
    ()
...
let fresh_var_spec (base: var_id) (avoid: list var_id)
  ...
  if mem base avoid then
    (* If base is in avoid, fresh_var calls fresh_var_helper which returns
       something not in avoid by the helper lemma. *)
    fresh_var_helper_spec base avoid 0
  else
    (* If base is not in avoid, fresh_var returns base directly.
       Since not (mem base avoid), the post-condition holds trivially. *)
    ()
...
let rec rename_expr (old_var new_var: var_id) (e: expr) : Tot expr (decreases expr_size e) =
  ...
  let new_inner =
    ...
    | ECopy dst src -> ECopy (rename_expr old_var new_var dst) (rename_expr old_var new_var src)
    | EClear e' -> EClear (rename_expr old_var new_var e')
    | EComplex r i -> EComplex (rename_expr old_var new_var r) (rename_expr old_var new_var i)
    | ERealPart e' -> ERealPart (rename_expr old_var new_var e')
    | EImagPart e' -> EImagPart (rename_expr old_var new_var e')
    | EConvert t e' -> EConvert t (rename_expr old_var new_var e')
    | ESliceFromPtr ptr_e len_e elem_ty ->
        ESliceFromPtr (rename_expr old_var new_var ptr_e) (rename_expr old_var new_var len_e) elem_ty
    | ESliceData e' -> ESliceData (rename_expr old_var new_var e')
    | EStringFromPtr ptr_e len_e ->
        EStringFromPtr (rename_expr old_var new_var ptr_e) (rename_expr old_var new_var len_e)
    | EStringData e' -> EStringData (rename_expr old_var new_var e')
    | EDelete map_e key_e -> EDelete (rename_expr old_var new_var map_e) (rename_expr old_var new_var key_e)
    | EPtrAdd ptr_e len_e -> EPtrAdd (rename_expr old_var new_var ptr_e) (rename_expr old_var new_var len_e)
    | EAppend slice elems ->
        EAppend (rename_expr old_var new_var slice)
                (List.Tot.map (rename_expr old_var new_var) elems)
    | EMake ty args ->
        EMake ty (List.Tot.map (rename_expr old_var new_var) args)
    | EPtrToInt e' -> EPtrToInt (rename_expr old_var new_var e')
    | EIntToPtr e' ty -> EIntToPtr (rename_expr old_var new_var e') ty
    | EMin args -> EMin (List.Tot.map (rename_expr old_var new_var) args)
    | EMax args -> EMax (List.Tot.map (rename_expr old_var new_var) args)
    (* Gradual typing cast *)
    | ECast e' ci -> ECast (rename_expr old_var new_var e') ci
...
let rec subst_expr (var: var_id) (replacement: expr) (target: expr)
  ...
  let new_inner =
    ...
    | ECopy dst src -> ECopy (subst_expr var replacement dst) (subst_expr var replacement src)
    | EClear e' -> EClear (subst_expr var replacement e')
    | EComplex r i -> EComplex (subst_expr var replacement r) (subst_expr var replacement i)
    | ERealPart e' -> ERealPart (subst_expr var replacement e')
    | EImagPart e' -> EImagPart (subst_expr var replacement e')
    | EConvert t e' -> EConvert t (subst_expr var replacement e')
    | ESliceFromPtr ptr_e len_e elem_ty ->
        ESliceFromPtr (subst_expr var replacement ptr_e) (subst_expr var replacement len_e) elem_ty
    | ESliceData e' -> ESliceData (subst_expr var replacement e')
    | EStringFromPtr ptr_e len_e ->
        EStringFromPtr (subst_expr var replacement ptr_e) (subst_expr var replacement len_e)
    | EStringData e' -> EStringData (subst_expr var replacement e')
    | EDelete map_e key_e -> EDelete (subst_expr var replacement map_e) (subst_expr var replacement key_e)
    | EPtrAdd ptr_e len_e -> EPtrAdd (subst_expr var replacement ptr_e) (subst_expr var replacement len_e)
    | EAppend slice elems ->
        EAppend (subst_expr var replacement slice)
                (List.Tot.map (subst_expr var replacement) elems)
    | EMake ty args ->
        EMake ty (List.Tot.map (subst_expr var replacement) args)
    | EPtrToInt e' -> EPtrToInt (subst_expr var replacement e')
    | EIntToPtr e' ty -> EIntToPtr (subst_expr var replacement e') ty
    | EMin args -> EMin (List.Tot.map (subst_expr var replacement) args)
    | EMax args -> EMax (List.Tot.map (subst_expr var replacement) args)
    (* Gradual typing cast *)
    | ECast e' ci -> ECast (subst_expr var replacement e') ci
...
(** ============================================================================
    ...
    DESIGN: Bidirectional Type Inference Interface
    ===============================================
    The type inference signatures define a bidirectional type checking pattern:

    infer_expr : type_env -> expr -> infer_result
      INFERENCE mode: Synthesize the type of an expression from context.
      Returns either (ty, effects) or an error with source location.

    check_expr : type_env -> expr -> brrr_type -> infer_result
      CHECKING mode: Verify expression has expected type.
      Can be more efficient than inference when expected type is known.

    This follows the standard bidirectional pattern used in:
    - F* itself (FStarC.BrrrTypeChecker.Tc.tc_term)
    - EverParse (Binding.fst check_expr)
    - HACL* (Lib.Buffer type annotations)

    ENVIRONMENT THREADING:
    ----------------------
    type_env = list (var_id & brrr_type) is an association list from
    variable names to their types. This is a simple representation that
    supports shadowing (later bindings override earlier ones).

    For performance, a production implementation would use:
    - Hash maps for O(1) lookup
    - Persistent data structures for efficient copying
    - Scope levels for efficient shadowing

    REFERENCE: fstar_pop_book.md lines 6776-6850 (typing environments)
               EverParse Binding.fst (environment threading pattern)
    ============================================================================ *)
...
(** ============================================================================
    ...
    DESIGN RATIONALE: Declarative Definition via Normalization
    ==========================================================
    There are multiple ways to define alpha equivalence:

    1. NOMINAL: Track variable names, define equivalence via renaming
       - Complex, requires explicit freshness conditions
       - Example: Nominal Isabelle, early F* formulations

    2. DE BRUIJN: Replace names with indices, compare structurally
       - Requires converting to/from de Bruijn representation
       - Clean but loses original variable names

    3. NORMALIZATION: Define a canonical form, compare normalized versions
       - This is our approach: expr_alpha_eq e1 e2 = expr_eq (normalize e1) (normalize e2)
       - Pros: Simple definition, easy to prove properties
       - Cons: Requires idempotence of normalize, which we prove

    Our normalization focuses on STRUCTURAL EQUIVALENCE (block flattening),
    not VARIABLE RENAMING. Full alpha-equivalence would require additionally:
    - Converting bound variables to a canonical form
    - Handling variable shadowing consistently

    For now, we focus on block normalization because:
    - It's the most common source of superficially different but semantically
      identical expressions (e.g., { { x } } vs { x } vs x)
    - Variable renaming alpha-equivalence is less common in our use cases
    ============================================================================ *)
...
let normalize_expr_idempotent (e: expr)
  ...
  admit()  (* Proven in BrrrExpressions.Theorems.fst *)
...
let rec expr_to_string (e: expr) : string =
  ...
  | EMake _ args ->
      "make(<type>, " ^ String.concat ", " (map expr_to_string args) ^ ")"
  | ENew _ -> "new(<type>)"
  | EHole -> "_"
