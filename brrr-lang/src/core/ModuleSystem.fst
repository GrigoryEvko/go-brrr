 ...
 *
 * ML-style module system for Brrr-Lang IR.
 * Based on brrr_lang_spec_v0.4.tex Part VIII (Module System).
 *
 * Features:
 *   - Module signatures with type, value, and effect members
 *   - First-class modules and functors
 *   - Signature subtyping with covariance/contravariance
 *   - Sealing for abstraction (opaque types)
...
(** ============================================================================
    SOURCE LOCATION TRACKING (following EverParse pattern)
    ============================================================================ *)
...
(** ============================================================================
    MODULE VISIBILITY (following EverParse export pattern)
    ============================================================================ *)

(* Module member visibility levels.
 *
 * Public   : Visible to all modules (exported in signature)
 ...
 *
 * Visibility ordering: Public > Internal > Private
 * A more permissive visibility subsumes a more restrictive one.
 *)
...
(** ============================================================================
    SHADOW CHECKING (following EverParse pattern)
    ============================================================================ *)
 ...
 *
 * Contains:
 * - Type members (abstract or manifest)
 * - Value members with polymorphic types
 * - Effect members with their signatures
 *
 * Order matters: later members can reference earlier ones.
 *)
noeq type signature = {
...
(* Functor mode determines type sharing semantics:
 *
 * Applicative: F(M) = F(M') when M equiv M'
 *   - Types are shared across applications
 *   - F(M).t = F(M).t (type equality preserved)
 *   - Default mode for most functors
 *
 * Generative: F(M) <> F(M') even when M equiv M'
 *   - Fresh abstract types on each application
 *   - Used for stateful modules, generativity
 *   - Each application creates new type identity
 *)
type functor_mode =
  | Applicative  (* functor^app - types shared across applications *)
 ...
 *
 * MTSig s     : A module with signature s
 * MTFunctor X S1 S2 mode : A functor from modules of type S1 to modules of type S2
 *                          X is the parameter name (for self-reference in S2)
 *                          mode is Applicative or Generative
 *)
 ...
 *
 * (M : S) where M : T and T <: S
 * Result has type S (with opaque types truly abstract)
 *)
...
(** ============================================================================
    SIGNATURE TYPE REFINEMENT (SigWhere)
    ============================================================================

    Implements "Sigma where type p = tau" (type sharing/refinement)

    Semantics:
    - Takes a signature Sigma with an abstract type named p
    - Refines it to make p transparent with definition tau
    - Essential for functor application type sharing

    Per spec (brrr_lang_spec_v0.4):
      SigWhere : signature -> string -> brrr_type -> signature

    Use cases:
    1. Functor application: F(M) where result types share with M's types
    2. Explicit type sharing: module N : S where type t = int
    3. Applicative functor semantics: F(M).t = F(M).t preservation
    ============================================================================ *)
...
(** ============================================================================
    ...
    Implements mixin composition per brrr_lang_spec_v0.4 Section 4:
    - Include: include M - incorporates all bindings
    - Linear composition: M1 + M2 - disjoint signature union
    - Override: M with { overrides } - refine components
    - Diamond resolution: explicit conflict resolution
    - Mixin type: { provided; required } signatures

    Mixin semantics:
    - A mixin is a partial module: it provides some bindings and requires others
    - Mixins compose via linear combination when signatures are disjoint
    - Conflicts (diamond problem) require explicit resolution via override
    - Include flattens a module into the current scope

    Brrr-Machine Analysis:
    - Detect diamond conflicts during composition
    - Track mixin provenance for dead code analysis
    - Verify required bindings are satisfied
    ============================================================================ *)
...
(** ============================================================================
    ...
    For brrr-machine analysis, we need to:
    1. Build a dependency graph between modules
    2. Detect circular dependencies (which are errors)
    3. Compute topological sort for compilation order

    The dependency graph is represented as adjacency lists where each module
    maps to a list of modules it depends on (imports from).
    ============================================================================ *)
...
noeq type dfs_state = {
  ds_visited  : list module_name;   (* Fully processed nodes *)
  ds_in_stack : list module_name;   (* Nodes in current DFS path *)
