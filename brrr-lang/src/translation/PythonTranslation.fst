 ...
 *
 * Complete translation functor from Python to Brrr-Lang IR.
 * Based on brrr_lang_spec_v0.4.md Part VI Section 2 (Python Translation).
 *
 * Translation rules T_Py:
 *   T_Py(None) = Unit
 *   T_Py(bool) = Bool
 *   T_Py(int) = Int[BigInt, Signed]
 ...
 *   T_Py(Any) = Dynamic
 *
 * Default effect: e_Py = <Throw | <IO | rowvar>>
 *
...
(** ============================================================================
    ...
    Types are imported from PythonTypes.fst (single source of truth).

    See PythonTypes.fst for the canonical py_type definition including:
...
(** ============================================================================
    TYPE TRANSLATION T_Py
    ============================================================================

    Translation from Python types to Brrr-Lang types.
...
let rec translate_py_type (t: py_type) : Tot brrr_type (decreases t) =
  ...
  | PyDict k v ->
      ...
      TStruct {
        struct_name = "PyDict";
        struct_fields = [
          { field_name = "_keys"; field_ty = t_array (translate_py_type k); field_vis = Private };
          { field_name = "_values"; field_ty = t_array (translate_py_type v); field_vis = Private }
        ];
  ...
  | PyGenerator yield_ty send_ty return_ty ->
      ...
      TFunc {
        ...
        ret = TStruct {
          ...
          struct_fields = [
            { field_name = "_yield_type"; field_ty = yield'; field_vis = Private };
            { field_name = "_send_type"; field_ty = send'; field_vis = Private };
            { field_name = "_return_type"; field_ty = return'; field_vis = Private }
          ];
          struct_repr = ReprRust
        };
        effects = gen_effects
...
(** ============================================================================
    ...
    Python expression AST is imported from PythonTypes.fst (single source of truth).

    See PythonTypes.fst for the canonical py_expr definition including:
...
(** ============================================================================
    EXPRESSION TRANSLATION T_Py
    ============================================================================

    Translation from Python expressions to Brrr-Lang expressions.
...
(** Translation result type and helper functions are imported from PythonTypes.fst.
...
(** ============================================================================
    ...
    Implementation of the translation_functor interface from LangMapping.fst.

    The python_functor provides:
    - source_mode: python_mode (GC, dynamic types, nullable, untracked, async)
    - translate_type: identity (types already translated at boundary)
    - translate_expr: identity (expressions already translated)
    - translate_value: Python value semantics
    ============================================================================ *)
...
let python_functor : translation_functor = {
  (* Python's language mode from LangMapping.fst *)
  ...
  (* Expression translation: identity for already-translated expressions
     ...
     Cross-language calls use boundary guards (see LangMapping.fst). *)
...
(** ============================================================================
    SEMANTIC PRESERVATION PROOFS - REAL TYPE AND EXPRESSION PROPERTIES
    ============================================================================

    These proofs establish meaningful semantic properties of the Python
    translation beyond trivial reflexivity. Following HACL*/EverParse patterns:
    - Use SMTPat for automatic lemma application
    - Use calc blocks for equational reasoning where appropriate
    - Use Classical.forall_intro for universal quantification

    Reference: HACL* specs/lemmas/Spec.SHA2.Lemmas.fst for proof patterns.
    ============================================================================ *)
...
(** ============================================================================
    ...
    Guards for cross-language calls involving Python.
    ============================================================================ *)
...
(** Guard for Python values crossing to statically-typed languages
    ...
    because Python doesn't guarantee type safety.
*)
...
(** ============================================================================
    ...
    Demonstrates translating a Python function call to Rust.

    Python: def foo(x: int) -> str: ...
    Brrr-Lang: fn foo(x: Int[BigInt, Signed]) -> String [Throw | IO | epsilon]

    When calling from Python to Rust:
    - Python's implicit effects must be handled by Rust's Result type
    - Dynamic type x must be checked to be an integer
    ============================================================================ *)
...
(** ============================================================================
    ...
    These proofs establish that the Python translation functor preserves the
    operational semantics of Python programs. Following HACL* patterns:
    - Spec.Agile.Hash.fst: Refinement between spec and impl
    - Hacl.Hash.SHA2.fst: friend declarations for proof access
    - Spec.SHA2.Lemmas.fst: SMTPat for automatic application
